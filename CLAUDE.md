...
2. **Check the negative-results register (§4) first.** Twenty
   ...
   **§4.20 is the seventh**, and it is the only one that implements nothing:
   it audits a community-derived CONSTANT (`conversion_reference`) sitting
   inside a flag that ships ON, which `calibrate` does not neutralise the way
   it zeroes the community-derived signals. The leak is real and is worth
   ~−2.6pp / −0.6pp of LEVEL and no verdict, so every absolute leakage-free
   figure dated on or after 2026-08-14 needs that caveat and none of the
   entries needs its verdict revisited. Read it before quoting a coverage
   number as leakage-free, and before touching that key — the stale value in
   `_weights.yaml` is stale on purpose and the entry says why.
