...
- The headline agreement metric cannot be used to tune the model: both its targets are also model inputs, so it is maximised by deleting the model. Use the leakage-free measure in `smite.calibrate`. Twenty-two correct-looking improvements have been measured and shipped off because of this; see `docs/STATE.md` section 4 before re-attempting any of them.
