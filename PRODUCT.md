...
- The headline agreement metric cannot be used to tune the model: both its targets are also model inputs, so it is maximised by deleting the model. Use the leakage-free measure in `smite.calibrate`. Twenty-seven correct-looking improvements have been measured against it, and all but three shipped off (one shipped on; one implements nothing and audits a community-derived constant the leakage-free measure does not neutralise; one implements nothing and diagnoses why a pricing flag's coverage delta is partly a change to the efficiency signal's own scale). The newest of them is the one to read first: the leakage-free measure is structurally BLIND to a defect deciding 28% of Conquest core slots, because that decision lives inside the `win` signal the measure zeroes; see `docs/STATE.md` section 4 before re-attempting any of them.
...
- Test suites back the pipeline and viewer: 922 Python tests (`pipeline/smite/tests`), 775 viewer tests.
