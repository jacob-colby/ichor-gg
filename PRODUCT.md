...
- The headline agreement metric cannot be used to tune the model: both its targets are also model inputs, so it is maximised by deleting the model. Use the leakage-free measure in `smite.calibrate`. Twenty-three correct-looking improvements have been measured against it, and all but two shipped off (one shipped on; one implements nothing and audits a community-derived constant the leakage-free measure does not neutralise); see `docs/STATE.md` section 4 before re-attempting any of them.
