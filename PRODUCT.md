...
- The model is a working **filter** and not a working **ranker**: measured against a random-baseline control it finds community-worthy items ~7x better than chance (41.3% against a 5.7% random core), while its ordering skill inside the community's own item set is indistinguishable from zero. "Sensible items" is supported; "right order" is not.
...
- The headline agreement metric cannot be used to tune the model: both its targets are also model inputs, so it is maximised by deleting the model. Use the leakage-free measure in `smite.calibrate`. Twenty-nine correct-looking improvements have been measured against it, and all but three shipped off (one shipped on; one implements nothing and audits a community-derived constant the leakage-free measure does not neutralise; one implements nothing and diagnoses why a pricing flag's coverage delta is partly a change to the efficiency signal's own scale). Two things to read first. The leakage-free measure is structurally BLIND to a defect deciding 28% of Conquest core slots, because that decision lives inside the `win` signal the measure zeroes. And the newest entry is not a null at all: it is a live defect — one shipped build archetype has never consulted the penetration cap — recorded with the measurement showing why closing it the obvious way costs 53 of 73 builds. See `docs/STATE.md` section 4 before re-attempting any of them.
...
- Test suites back the pipeline and viewer: 923 Python tests (`pipeline/smite/tests`), 775 viewer tests.
