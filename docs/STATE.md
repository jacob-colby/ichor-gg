...
| Cap overflow | 47 -> 0, and back -> 2 of 2430 builds over the penetration cap. `cap_overflow` took the original 47 to 29; `price_adaptive` reshaped enough cores to clear the rest — measured, not designed — and `offmap_efficiency` at 0.55 put three back, which is the same mechanism in reverse: it reshapes cores toward penetration items, and `assemble._capped_out` still cannot refuse an item whose penetration alone is dead (register §4.2). Three of 2,428 is the smallest this figure has been apart from zero; it is reported, not tuned against. See `cap_overflow` |
