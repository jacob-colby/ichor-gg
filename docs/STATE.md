...
| Cap overflow | 47 -> 0, and back -> 3 of 2390 builds over the penetration cap. `cap_overflow` took the original 47 to 29; `price_adaptive` reshaped enough cores to clear the rest — measured, not designed — and `offmap_efficiency` at 0.55 put three back, which is the same mechanism in reverse: it reshapes cores toward penetration items, and `assemble._capped_out` still cannot refuse an item whose penetration alone is dead (register §4.2). Three of 2,428 is the smallest this figure has been apart from zero; it is reported, not tuned against. Both figures moved on the 25 Aug window reset (see Community sample) and neither is a model change: 40 suggested builds lost the data to exist at all (the index holds 2,479 where it held 2,519 — Conquest 886 against 899, Arena 796 against 812, Joust 797 against 808), which is the whole of the denominator's 2430 to 2390, and one more of the survivors sits over the cap. See `cap_overflow` |
