...
- **Nothing entered anywhere** collapses further, but not to a quiet text link — a first pass at that read as too easy to miss for what this session judged the app's central feature. It's the same footprint and shadow tier (`shadow-raised`) the started dock uses, opening on a literal miniature of the ten-slot board it links to (`Slot`'s own empty-state motifs at chip scale — solid `border-gold` for the you-slot, `border-dashed border-line-strong` for the rest — reused, not reinvented) above the `Start a draft →` line, so the invitation shows what it opens onto rather than just naming it.
