...
export function GodInfo({ god }: { god: God }) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {stats.length > 0 && (
        <div className="mb-5">
          <div className="mb-2 font-display text-small font-semibold tracking-widest text-muted">BASE STATS</div>
          <div className="rounded-md border border-line bg-bg2 p-3">
            <div className="grid grid-cols-[auto_1fr_1fr] gap-x-4 gap-y-1.5 text-body">
              <div className="text-micro uppercase tracking-wide text-faint">Stat</div>
              <div className="text-micro uppercase tracking-wide text-faint">Base</div>
              <div className="text-micro uppercase tracking-wide text-faint">/ level</div>
              {stats.map(([k, v]) => (
      ...
      <div className="mb-5">
        <div className="mb-2 font-display text-small font-semibold tracking-widest text-muted">ABILITIES</div>
        <div className="flex flex-col gap-3">
          {(god.abilities ?? []).map((a, i) => (
            <div key={`${a.name}-${i}`} className="rounded-md border border-line bg-bg2 p-3">
              <div className="flex items-baseline gap-2">
                <span className="text-micro uppercase tracking-wide text-faint">{a.slot}</span>
                <span className="font-display text-body font-semibold text-ink">{a.name}</span>
              </div>
              {a.description && <div className="mt-1 text-body text-ink-soft">{a.description}</div>}
              {a.details?.length ? (
                <div className="mt-1 flex flex-wrap gap-x-3 text-small text-muted">
      ...
      {god.ability_order && (
        <div className="mb-5">
          <div className="mb-2 font-display text-small font-semibold tracking-widest text-muted">ABILITY ORDER</div>
          <div className="rounded-md border border-line bg-bg2 p-3">
            <div className="grid grid-cols-10 gap-1">
              {god.ability_order.order.map((slot, i) => {
                ...
                return (
                  ...
                  >
                    <span className="text-micro text-faint">{level}</span>
            ...
            <div className="mt-1 text-micro italic text-faint">
      ...
      {god.aspects?.[0] && (
        <div>
          <div className="mb-2 font-display text-small font-semibold tracking-widest text-muted">ASPECT</div>
          <div className="rounded-md border border-gold/40 bg-bg2 p-3 text-body">
