...
const fieldCls = "w-full rounded-md border border-line bg-bg2 px-2.5 py-1.5 text-body text-ink placeholder:text-muted focus:border-blue focus:outline-none";
...
export function BuildEditor({ god, mode, items, starters, initial, defaultStarter, onClose, onSaved }: BuildEditorProps) {
  ...
  return (
    <div className="max-w-md rounded-lg border border-line bg-bg1 p-4 shadow-card">
      <div className="mb-3 font-display text-title font-semibold text-ink">
      ...
      <button
        ...
        className="press flex w-full items-center justify-center rounded-md border border-line bg-bg2 px-2.5 py-1.5 text-body text-blue transition-colors duration-[180ms] ease-standard hover:border-line-strong disabled:opacity-50"
      >
      ...
      <div className="my-3 flex flex-col gap-1">
        {slots.map((n, i) => (
          <div key={`${n}-${i}`} className="flex items-center gap-2 rounded-md border border-line bg-bg2 px-2 py-1.5 text-body text-ink">
            <span className="w-4 font-mono text-small text-faint">{i + 1}</span>
        ...
        {slots.length === 0 && <div className="text-small text-muted">No items yet (max 6).</div>}
      </div>
      ...
      {error && <div className="mb-2 text-small text-premium">{error}</div>}
      <div className="flex gap-2">
        <button type="button" onClick={save}
          className="press rounded-md bg-gold px-3 py-1.5 text-body font-semibold text-bg0">Save</button>
        {initial && (
          <button type="button" onClick={remove}
            className="press rounded-md bg-bg2 px-3 py-1.5 text-body text-premium hover:bg-line">Delete</button>
        )}
        <button type="button" onClick={onClose} className="press rounded-md bg-bg2 px-3 py-1.5 text-body text-muted hover:text-ink">Cancel</button>
      </div>
    </div>
  );
}
