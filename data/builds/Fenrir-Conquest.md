---
type: smite-build
god: Fenrir
mode: Conquest
builds:
- source: community
  aspect: Aspect of Loyalty
  aspect_pick_rate: 0.13
  aspect_win_rate: 0.4
  slot_order:
  - name: Jotunn's Revenge
    pick_rate: 0.42
    win_rate: 0.41
    alternates:
    - name: The Reaper
      pick_rate: 0.15
      win_rate: 0.7
    - name: Shifter's Shield
      pick_rate: 0.1
      win_rate: 0.41
  - name: Shifter's Shield
    pick_rate: 0.25
    win_rate: 0.47
    alternates:
    - name: Sanguine Lash
      pick_rate: 0.1
      win_rate: 0.67
    - name: The Reaper
      pick_rate: 0.07
      win_rate: 0.47
  - name: Kinetic Cuirass
    pick_rate: 0.12
    win_rate: 0.42
    alternates:
    - name: Shifter's Shield
      pick_rate: 0.12
      win_rate: 0.76
    - name: The Reaper
      pick_rate: 0.07
      win_rate: 0.4
  - name: Hide of the Nemean Lion
    pick_rate: 0.09
    win_rate: 0.7
    alternates:
    - name: Freya's Tears
      pick_rate: 0.08
      win_rate: 0.55
    - name: Heartseeker
      pick_rate: 0.07
      win_rate: 0.42
  - name: Shell of Rebuke
    pick_rate: 0.06
    win_rate: 0.8
    alternates:
    - name: Titan's Bane
      pick_rate: 0.06
      win_rate: 0.27
    - name: Kinetic Cuirass
      pick_rate: 0.05
      win_rate: 0.67
  - name: Engraved Guard
    pick_rate: 0.11
    win_rate: 0.56
    alternates:
    - name: Skeggox
      pick_rate: 0.05
      win_rate: 0.5
    - name: Kinetic Cuirass
      pick_rate: 0.05
      win_rate: 0.43
  community_starters:
  - name: Hunter's Cowl
    pick_rate: 0.26
    win_rate: 0.59
  - name: Bumba's Hammer
    pick_rate: 0.17
    win_rate: 0.59
  - name: Bumba's Cudgel
    pick_rate: 0.15
    win_rate: 0.4
  source_url: https://smitebrain.com/gods/fenrir/
  last_verified: '2026-09-19'
  god_win_rate: 0.48727272727272725
  god_matches_won: 134
  god_matches_played: 275
  god_division: obsidian
  god_window_start: '2026-09-08'
  god_window_end: '2026-09-19'
  god_matches_analyzed: 12806
  starter:
    base: Bumba's Golden Dagger
    upgrade: Bumba's Spear
- source: suggested
  archetype: core
  slot_order:
  - Jotunn's Revenge
  - Transcendence
  - Hydra's Lament
  - The Reaper
  - Pendulum Blade
  - The Crusher
  flex_slots:
  - Hydra's Lament
  - Transcendence
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Avatar's Parashu — CC-immunity / cleanse
    swap_item: Avatar's Parashu
  - vs_tag: magic_heavy
    swap: Sanguine Lash — magical protection
    swap_item: Sanguine Lash
  - vs_tag: physical_heavy
    swap: Shield Splitter — physical protection
    swap_item: Shield Splitter
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Top weighted-score core (efficiency + fit + win/pick). Underrated for
    this god: The Reaper, The Crusher, Pendulum Blade, Hydra''s Lament, Avatar''s
    Parashu, Tekko-Kagi, Heartseeker, Tyrfing, Arondight, Transcendence, Runeforged
    Hammer, Sanguine Lash, Avenging Blade, Golden Blade, Lernaean Bow, Shield Splitter,
    Dominance, Silverbranch Bow, Oath-Sworn Spear, Riptalon, Barbed Carver, Titan''s
    Bane, Toxic Blade, Bloodforge, Deathbringer, Eye of the Storm, Damaru, Rage, Musashi''s
    Dual Swords.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.61
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 1.0
    Transcendence:
      total: 0.5
      efficiency: 0.53
      win: 0.53
      pick: 0.0
      fit: 0.52
    Hydra's Lament:
      total: 0.53
      efficiency: 0.54
      win: 0.53
      pick: 0.0
      fit: 0.71
    The Reaper:
      total: 0.63
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.91
    Pendulum Blade:
      total: 0.53
      efficiency: 0.42
      win: 0.53
      pick: 0.0
      fit: 1.0
    The Crusher:
      total: 0.55
      efficiency: 0.47
      win: 0.53
      pick: 0.0
      fit: 1.0
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  starter: &id001
    base: Bumba's Golden Dagger
    upgrade: Bumba's Spear
- source: suggested
  archetype: mana-stack
  slot_order:
  - Jotunn's Revenge
  - Transcendence
  - Hydra's Lament
  - The Reaper
  - Arondight
  - The Crusher
  flex_slots:
  - Transcendence
  - Arondight
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Avatar's Parashu — CC-immunity / cleanse
    swap_item: Avatar's Parashu
  - vs_tag: magic_heavy
    swap: Sanguine Lash — magical protection
    swap_item: Sanguine Lash
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'mana-stack (efficiency + fit + win/pick). Underrated for this god: The
    Reaper, Hydra''s Lament, The Crusher, Transcendence, Arondight, Pendulum Blade,
    Heartseeker, Avatar''s Parashu, Runeforged Hammer, Tyrfing, Tekko-Kagi, Avenging
    Blade, Dominance, Breastplate of Valor, Lernaean Bow, Genji''s Guard, Shield Splitter,
    Golden Blade, Oath-Sworn Spear, Daybreak Gavel, Silverbranch Bow, Barbed Carver,
    Bloodforge, Riptalon, Yogi''s Necklace, Toxic Blade, Deathbringer, Eye of the
    Storm, Titan''s Bane.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.56
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 0.71
    Transcendence:
      total: 0.48
      efficiency: 0.53
      win: 0.53
      pick: 0.0
      fit: 0.39
    Hydra's Lament:
      total: 0.52
      efficiency: 0.54
      win: 0.53
      pick: 0.0
      fit: 0.63
    The Reaper:
      total: 0.57
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.47
    Arondight:
      total: 0.48
      efficiency: 0.5
      win: 0.53
      pick: 0.0
      fit: 0.43
    The Crusher:
      total: 0.49
      efficiency: 0.47
      win: 0.53
      pick: 0.0
      fit: 0.57
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  starter: *id001
- source: suggested
  archetype: burst
  slot_order:
  - Jotunn's Revenge
  - Transcendence
  - Hydra's Lament
  - The Reaper
  - Pendulum Blade
  - The Crusher
  flex_slots:
  - The Crusher
  - Transcendence
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Avatar's Parashu — CC-immunity / cleanse
    swap_item: Avatar's Parashu
  - vs_tag: magic_heavy
    swap: Sanguine Lash — magical protection
    swap_item: Sanguine Lash
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Ability / burst skew (efficiency + fit + win/pick). Underrated for this
    god: The Reaper, Hydra''s Lament, Pendulum Blade, The Crusher, Arondight, Avatar''s
    Parashu, Heartseeker, Tekko-Kagi, Transcendence, Runeforged Hammer, Tyrfing, Avenging
    Blade, Silverbranch Bow, Breastplate of Valor, Riptalon, Genji''s Guard, Lernaean
    Bow, Toxic Blade, Shield Splitter, Dominance, Daybreak Gavel, Golden Blade, Oath-Sworn
    Spear, Barbed Carver, Eye of Erebus, Screeching Gargoyle, Bloodforge, Chandra''s
    Grace, Titan''s Bane.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.57
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 0.78
    Transcendence:
      total: 0.45
      efficiency: 0.53
      win: 0.53
      pick: 0.0
      fit: 0.22
    Hydra's Lament:
      total: 0.51
      efficiency: 0.54
      win: 0.53
      pick: 0.0
      fit: 0.54
    The Reaper:
      total: 0.58
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.56
    Pendulum Blade:
      total: 0.5
      efficiency: 0.42
      win: 0.53
      pick: 0.0
      fit: 0.78
    The Crusher:
      total: 0.5
      efficiency: 0.47
      win: 0.53
      pick: 0.0
      fit: 0.66
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  starter: *id001
- source: suggested
  archetype: bruiser
  slot_order:
  - Jotunn's Revenge
  - Shell of Rebuke
  - Hide of the Nemean Lion
  - The Reaper
  - Freya's Tears
  - Amanita Charm
  flex_slots:
  - Jotunn's Revenge
  - Shell of Rebuke
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Magi's Cloak — CC-immunity / cleanse
    swap_item: Magi's Cloak
  - vs_tag: magic_heavy
    swap: Shield Splitter — magical protection
    swap_item: Shield Splitter
  - vs_tag: physical_heavy
    swap: Shield of the Phoenix — physical protection
    swap_item: Shield of the Phoenix
  - vs_tag: sustain
    swap: Stygian Anchor — anti-heal
    swap_item: Stygian Anchor
  rationale: 'Lifesteal bruiser skew (efficiency + fit + win/pick). Underrated for
    this god: The Reaper, Amanita Charm, Shield of the Phoenix, Freya''s Tears, Runeforged
    Hammer, Shield Splitter, Eye of the Storm, Berserker''s Shield, Erosion, Eye of
    Providence, Genji''s Guard, Breastplate of Valor, Draconic Scale, Yogi''s Necklace,
    Phoenix Feather, Avenging Blade, Hydra''s Lament, Stone of Binding, Midgardian
    Mail, Chandra''s Grace, Daybreak Gavel, The Crusher, Magi''s Cloak, Leviathan''s
    Hide, Heartseeker, Titan''s Bane.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.52
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 0.44
    Shell of Rebuke:
      total: 0.52
      efficiency: 0.28
      win: 0.8
      pick: 0.13
      fit: 0.34
    Hide of the Nemean Lion:
      total: 0.56
      efficiency: 0.52
      win: 0.7
      pick: 0.15
      fit: 0.39
    The Reaper:
      total: 0.6
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.7
    Freya's Tears:
      total: 0.53
      efficiency: 0.61
      win: 0.55
      pick: 0.13
      fit: 0.38
    Amanita Charm:
      total: 0.59
      efficiency: 0.65
      win: 0.53
      pick: 0.0
      fit: 0.86
  community_ordered:
  - Jotunn's Revenge
  - Shell of Rebuke
  - Hide of the Nemean Lion
  - The Reaper
  - Freya's Tears
  starter: *id001
- source: suggested
  archetype: anti-tank
  slot_order:
  - Jotunn's Revenge
  - The Reaper
  - Tekko-Kagi
  - Pendulum Blade
  - The Crusher
  - Avatar's Parashu
  flex_slots:
  - Pendulum Blade
  - Tekko-Kagi
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Stampede — CC-immunity / cleanse
    swap_item: Stampede
  - vs_tag: magic_heavy
    swap: Sanguine Lash — magical protection
    swap_item: Sanguine Lash
  - vs_tag: physical_heavy
    swap: Shield Splitter — physical protection
    swap_item: Shield Splitter
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Full-penetration anti-tank skew (efficiency + fit + win/pick). Underrated
    for this god: The Reaper, The Crusher, Avatar''s Parashu, Pendulum Blade, Tekko-Kagi,
    Heartseeker, Avenging Blade, Hydra''s Lament, Silverbranch Bow, Riptalon, Oath-Sworn
    Spear, Tyrfing, Transcendence, Arondight, Toxic Blade, Runeforged Hammer, Titan''s
    Bane, Lernaean Bow, Shield Splitter, Dominance, Golden Blade, Barbed Carver, Screeching
    Gargoyle, Daybreak Gavel, Bloodforge, Breastplate of Valor, Genji''s Guard, Deathbringer,
    Eye of the Storm.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.61
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 1.0
    The Reaper:
      total: 0.64
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.94
    Tekko-Kagi:
      total: 0.52
      efficiency: 0.41
      win: 0.53
      pick: 0.0
      fit: 0.94
    Pendulum Blade:
      total: 0.53
      efficiency: 0.42
      win: 0.53
      pick: 0.0
      fit: 1.0
    The Crusher:
      total: 0.55
      efficiency: 0.47
      win: 0.53
      pick: 0.0
      fit: 1.0
    Avatar's Parashu:
      total: 0.54
      efficiency: 0.45
      win: 0.53
      pick: 0.0
      fit: 0.94
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  starter: *id001
- source: suggested
  archetype: attack-speed
  slot_order:
  - Jotunn's Revenge
  - Tyrfing
  - The Reaper
  - Riptalon
  - Silverbranch Bow
  - Tekko-Kagi
  flex_slots:
  - Jotunn's Revenge
  - Tekko-Kagi
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Avatar's Parashu — CC-immunity / cleanse
    swap_item: Avatar's Parashu
  - vs_tag: magic_heavy
    swap: Sanguine Lash — magical protection
    swap_item: Sanguine Lash
  - vs_tag: physical_heavy
    swap: Berserker's Shield — physical protection
    swap_item: Berserker's Shield
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Basic-attack DPS skew (efficiency + fit + win/pick). Underrated for
    this god: The Reaper, Riptalon, Tyrfing, Silverbranch Bow, Tekko-Kagi, Lernaean
    Bow, Golden Blade, Toxic Blade, Dominance, Qin''s Blade, Hydra''s Lament, The
    Crusher, Sun Beam Bow, Transcendence, Berserker''s Shield, Avatar''s Parashu,
    Dagger of Frenzy, Arondight, Runeforged Hammer, Pendulum Blade, Heartseeker, Avenging
    Blade, Barbed Carver, Vital Amplifier, Hastened Fatalis, Bloodforge, The Executioner,
    Odysseus'' Bow, Titan''s Bane.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.51
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 0.37
    Tyrfing:
      total: 0.52
      efficiency: 0.48
      win: 0.53
      pick: 0.0
      fit: 0.79
    The Reaper:
      total: 0.58
      efficiency: 0.55
      win: 0.7
      pick: 0.15
      fit: 0.44
    Riptalon:
      total: 0.53
      efficiency: 0.51
      win: 0.53
      pick: 0.0
      fit: 0.79
    Silverbranch Bow:
      total: 0.52
      efficiency: 0.53
      win: 0.53
      pick: 0.0
      fit: 0.69
    Tekko-Kagi:
      total: 0.5
      efficiency: 0.49
      win: 0.53
      pick: 0.0
      fit: 0.65
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  starter: *id001
- source: suggested
  archetype: cooldown
  slot_order:
  - Jotunn's Revenge
  - Transcendence
  - Hydra's Lament
  - The Reaper
  - Arondight
  - Pendulum Blade
  flex_slots:
  - Arondight
  - Transcendence
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Avatar's Parashu — CC-immunity / cleanse
    swap_item: Avatar's Parashu
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Ability-uptime skew — Cooldown Rate is a rate, not a reduction (efficiency
    + fit + win/pick). Underrated for this god: The Reaper, Hydra''s Lament, Pendulum
    Blade, Arondight, Breastplate of Valor, Genji''s Guard, The Crusher, Freya''s
    Tears, Avatar''s Parashu, Eye of Erebus, Transcendence, Heartseeker, Tyrfing,
    Screeching Gargoyle, Runeforged Hammer, Chandra''s Grace, Tekko-Kagi, Avenging
    Blade, Shield of the Phoenix, Silverbranch Bow, Daybreak Gavel, Lernaean Bow,
    Riptalon, Gladiator''s Shield, Shield Splitter, Dominance, Golden Blade, Prophetic
    Cloak, Titan''s Bane.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.58
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 0.85
    Transcendence:
      total: 0.44
      efficiency: 0.53
      win: 0.53
      pick: 0.0
      fit: 0.15
    Hydra's Lament:
      total: 0.54
      efficiency: 0.54
      win: 0.53
      pick: 0.0
      fit: 0.75
    The Reaper:
      total: 0.55
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.35
    Arondight:
      total: 0.51
      efficiency: 0.5
      win: 0.53
      pick: 0.0
      fit: 0.65
    Pendulum Blade:
      total: 0.51
      efficiency: 0.42
      win: 0.53
      pick: 0.0
      fit: 0.85
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  starter: *id001
- source: suggested
  archetype: model
  slot_order:
  - Jotunn's Revenge
  - The Reaper
  - Pendulum Blade
  - The Crusher
  - Heartseeker
  - Titan's Bane
  flex_slots:
  - The Reaper
  - Pendulum Blade
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Avatar's Parashu — CC-immunity / cleanse
    swap_item: Avatar's Parashu
  - vs_tag: magic_heavy
    swap: Shield Splitter — magical protection
    swap_item: Shield Splitter
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'The model''s own answer — no meta signal (efficiency + fit + win/pick).
    Underrated for this god: Titan''s Bane, The Crusher, Heartseeker, The Reaper,
    Pendulum Blade, Hydra''s Lament, Avatar''s Parashu, Tekko-Kagi, Tyrfing, Arondight,
    Transcendence, Runeforged Hammer, Avenging Blade, Golden Blade, Lernaean Bow,
    Shield Splitter, Dominance, Silverbranch Bow, Oath-Sworn Spear, Riptalon, Barbed
    Carver, Toxic Blade, Bloodforge, Deathbringer, Eye of the Storm, Damaru, Rage,
    Musashi''s Dual Swords, Sanguine Lash.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.61
      efficiency: 0.72
      win: 0.41
      pick: 0.42
      fit: 1.0
    The Reaper:
      total: 0.63
      efficiency: 0.5
      win: 0.7
      pick: 0.15
      fit: 0.91
    Pendulum Blade:
      total: 0.53
      efficiency: 0.42
      win: 0.53
      pick: 0.0
      fit: 1.0
    The Crusher:
      total: 0.55
      efficiency: 0.47
      win: 0.53
      pick: 0.0
      fit: 1.0
    Heartseeker:
      total: 0.51
      efficiency: 0.47
      win: 0.42
      pick: 0.12
      fit: 1.0
    Titan's Bane:
      total: 0.44
      efficiency: 0.47
      win: 0.27
      pick: 0.13
      fit: 1.0
  community_ordered:
  - Jotunn's Revenge
  - The Reaper
  - Heartseeker
  - Titan's Bane
  starter: *id001
---
