---
type: smite-build
god: Baron Samedi
mode: Conquest
builds:
- source: community
  aspect: Aspect of Hysteria
  aspect_pick_rate: 0.83
  aspect_win_rate: 0.65
  slot_order:
  - name: Daybreak Gavel
    pick_rate: 0.19
    win_rate: 0.64
    alternates:
    - name: Shifter's Shield
      pick_rate: 0.14
      win_rate: 0.88
    - name: Spear of Desolation
      pick_rate: 0.12
      win_rate: 0.43
  - name: Genji's Guard
    pick_rate: 0.14
    win_rate: 0.63
    alternates:
    - name: Shifter's Shield
      pick_rate: 0.12
      win_rate: 0.71
    - name: Breastplate of Valor
      pick_rate: 0.1
      win_rate: 0.67
  - name: Breastplate of Valor
    pick_rate: 0.11
    win_rate: 0.67
    alternates:
    - name: Genji's Guard
      pick_rate: 0.11
      win_rate: 0.5
    - name: Shell of Rebuke
      pick_rate: 0.11
      win_rate: 0.67
  - name: Rod of Tahuti
    pick_rate: 0.15
    win_rate: 0.5
    alternates:
    - name: Freya's Tears
      pick_rate: 0.13
      win_rate: 0.71
    - name: Shell of Rebuke
      pick_rate: 0.08
      win_rate: 0.75
  - name: Shell of Rebuke
    pick_rate: 0.09
    win_rate: 0.5
    alternates:
    - name: Genji's Guard
      pick_rate: 0.09
      win_rate: 1.0
    - name: Ethereal Staff
      pick_rate: 0.06
      win_rate: 0.67
  - name: Shield
    pick_rate: 0.08
    win_rate: 0.5
    alternates:
    - name: Obsidian Shard
      pick_rate: 0.08
      win_rate: 1.0
    - name: Legionnaire Armor
      pick_rate: 0.08
      win_rate: 0.5
  community_starters:
  - name: Bluestone Pendant
    pick_rate: 0.28
    win_rate: 0.44
  - name: Bluestone Brooch
    pick_rate: 0.22
    win_rate: 0.62
  - name: Archmage's Gem
    pick_rate: 0.1
    win_rate: 0.83
  source_url: https://smitebrain.com/gods/baron-samedi/
  last_verified: '2026-08-13'
  god_win_rate: 0.603448275862069
  god_matches_won: 35
  god_matches_played: 58
  god_division: obsidian
  god_window_start: '2026-08-11'
  god_window_end: '2026-08-13'
  god_matches_analyzed: 2102
  starter:
    base: Conduit Gem
    upgrade: Archmage's Gem
- source: suggested
  archetype: core
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Freya's Tears
  - Gluttonous Grimoire
  - Shifter's Shield
  - Obsidian Shard
  flex_slots:
  - Genji's Guard
  - Shifter's Shield
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Kinetic Cuirass — magical protection
    swap_item: Kinetic Cuirass
  - vs_tag: physical_heavy
    swap: Amanita Charm — physical protection
    swap_item: Amanita Charm
  - vs_tag: sustain
    swap: Stygian Anchor — anti-heal
    swap_item: Stygian Anchor
  rationale: 'Top weighted-score core (efficiency + fit + win/pick). Underrated for
    this god: Gluttonous Grimoire, Soul Gem, Kinetic Cuirass, Amanita Charm, Shield
    of the Phoenix, Spear of the Magus, Oni Hunter''s Garb, The Cosmic Horror, Ethereal
    Staff, Helm of Radiance, Erosion, Eye of Providence, Spectral Armor, Rod of Asclepius,
    Draconic Scale, Chronos'' Pendant, Jade Scepter, Leviathan''s Hide, Wish-Granting
    Pearl, Midgardian Mail, Mantle Of Discord, Spear of Desolation.'
  slot_scores:
    Genji's Guard:
      total: 0.59
      efficiency: 0.71
      win: 0.63
      pick: 0.19
      fit: 0.31
    Breastplate of Valor:
      total: 0.59
      efficiency: 0.66
      win: 0.67
      pick: 0.17
      fit: 0.31
    Freya's Tears:
      total: 0.61
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.48
    Gluttonous Grimoire:
      total: 0.61
      efficiency: 0.66
      win: 0.64
      pick: 0.0
      fit: 0.62
    Shifter's Shield:
      total: 0.58
      efficiency: 0.52
      win: 0.71
      pick: 0.16
      fit: 0.49
    Obsidian Shard:
      total: 0.71
      efficiency: 0.54
      win: 1.0
      pick: 0.25
      fit: 0.37
  starter: &id001
    base: Conduit Gem
    upgrade: Archmage's Gem
- source: suggested
  archetype: burst
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Freya's Tears
  - Gluttonous Grimoire
  - Shifter's Shield
  - Obsidian Shard
  flex_slots:
  - Genji's Guard
  - Shifter's Shield
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Kinetic Cuirass — magical protection
    swap_item: Kinetic Cuirass
  - vs_tag: physical_heavy
    swap: Amanita Charm — physical protection
    swap_item: Amanita Charm
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Ability / burst skew (efficiency + fit + win/pick). Underrated for this
    god: Gluttonous Grimoire, Soul Gem, Kinetic Cuirass, Amanita Charm, Spear of the
    Magus, The Cosmic Horror, Shield of the Phoenix, Oni Hunter''s Garb, Death Metal,
    Helm of Radiance, Spectral Armor, Chronos'' Pendant, Erosion, Rod of Asclepius,
    Eye of Providence, Jade Scepter, Draconic Scale, Leviathan''s Hide, Screeching
    Gargoyle, Blood-Bound Book, Midgardian Mail, Spear of Desolation.'
  slot_scores:
    Genji's Guard:
      total: 0.58
      efficiency: 0.71
      win: 0.63
      pick: 0.19
      fit: 0.27
    Breastplate of Valor:
      total: 0.58
      efficiency: 0.66
      win: 0.67
      pick: 0.17
      fit: 0.27
    Freya's Tears:
      total: 0.6
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.39
    Gluttonous Grimoire:
      total: 0.6
      efficiency: 0.66
      win: 0.64
      pick: 0.0
      fit: 0.56
    Shifter's Shield:
      total: 0.56
      efficiency: 0.52
      win: 0.71
      pick: 0.16
      fit: 0.37
    Obsidian Shard:
      total: 0.71
      efficiency: 0.54
      win: 1.0
      pick: 0.25
      fit: 0.35
  starter: *id001
- source: suggested
  archetype: bruiser
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Freya's Tears
  - Obsidian Shard
  - Amanita Charm
  - Soul Gem
  flex_slots:
  - Breastplate of Valor
  - Genji's Guard
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Shifter's Shield — magical protection
    swap_item: Shifter's Shield
  - vs_tag: physical_heavy
    swap: Shield of the Phoenix — physical protection
    swap_item: Shield of the Phoenix
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Lifesteal bruiser skew (efficiency + fit + win/pick). Underrated for
    this god: Soul Gem, Amanita Charm, Gluttonous Grimoire, Shield of the Phoenix,
    Rod of Asclepius, Kinetic Cuirass, Blood-Bound Book, Chandra''s Grace, Bancroft''s
    Talon, Oni Hunter''s Garb, Spear of the Magus, Lifebinder, The Cosmic Horror,
    Phoenix Feather, Helm of Radiance, Erosion, Spectral Armor, Jade Scepter, Eye
    of Providence, Draconic Scale, Chronos'' Pendant, Spear of Desolation.'
  slot_scores:
    Genji's Guard:
      total: 0.58
      efficiency: 0.71
      win: 0.63
      pick: 0.19
      fit: 0.29
    Breastplate of Valor:
      total: 0.58
      efficiency: 0.66
      win: 0.67
      pick: 0.17
      fit: 0.29
    Freya's Tears:
      total: 0.6
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.44
    Obsidian Shard:
      total: 0.71
      efficiency: 0.54
      win: 1.0
      pick: 0.25
      fit: 0.37
    Amanita Charm:
      total: 0.62
      efficiency: 0.63
      win: 0.64
      pick: 0.0
      fit: 0.79
    Soul Gem:
      total: 0.63
      efficiency: 0.59
      win: 0.64
      pick: 0.0
      fit: 0.91
  starter: *id001
- source: suggested
  archetype: anti-tank
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Freya's Tears
  - Gluttonous Grimoire
  - Shifter's Shield
  - Obsidian Shard
  flex_slots:
  - Genji's Guard
  - Shifter's Shield
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Kinetic Cuirass — magical protection
    swap_item: Kinetic Cuirass
  - vs_tag: physical_heavy
    swap: Amanita Charm — physical protection
    swap_item: Amanita Charm
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Full-penetration anti-tank skew (efficiency + fit + win/pick). Underrated
    for this god: Gluttonous Grimoire, Soul Gem, Kinetic Cuirass, Spear of the Magus,
    Amanita Charm, The Cosmic Horror, Stone of Binding, Screeching Gargoyle, Void
    Shield, Oni Hunter''s Garb, Shield of the Phoenix, Void Stone, Helm of Radiance,
    Spectral Armor, The World Stone, Doom Orb, Erosion, Rod of Asclepius, Dreamer''s
    Idol, Eye of Providence, Chronos'' Pendant, Spear of Desolation.'
  slot_scores:
    Genji's Guard:
      total: 0.58
      efficiency: 0.71
      win: 0.63
      pick: 0.19
      fit: 0.26
    Breastplate of Valor:
      total: 0.58
      efficiency: 0.66
      win: 0.67
      pick: 0.17
      fit: 0.26
    Freya's Tears:
      total: 0.6
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.4
    Gluttonous Grimoire:
      total: 0.62
      efficiency: 0.66
      win: 0.64
      pick: 0.0
      fit: 0.7
    Shifter's Shield:
      total: 0.57
      efficiency: 0.52
      win: 0.71
      pick: 0.16
      fit: 0.41
    Obsidian Shard:
      total: 0.72
      efficiency: 0.54
      win: 1.0
      pick: 0.25
      fit: 0.48
  starter: *id001
- source: suggested
  archetype: attack-speed
  slot_order:
  - Bracer of The Abyss
  - Nimble Ring
  - Bragi's Harp
  - Freya's Tears
  - Gluttonous Grimoire
  - Obsidian Shard
  flex_slots:
  - Bragi's Harp
  - Nimble Ring
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Basic-attack DPS skew (efficiency + fit + win/pick). Underrated for
    this god: Gluttonous Grimoire, Soul Gem, Kinetic Cuirass, Amanita Charm, Oni Hunter''s
    Garb, Spear of the Magus, Bracer of The Abyss, Shield of the Phoenix, The Cosmic
    Horror, Helm of Radiance, Death Metal, Spectral Armor, Blood-Bound Book, Rod of
    Asclepius, Bancroft''s Talon, Chronos'' Pendant, Jade Scepter, Erosion, Bragi''s
    Harp, Eye of Providence, Nimble Ring, Spear of Desolation.'
  slot_scores:
    Bracer of The Abyss:
      total: 0.52
      efficiency: 0.56
      win: 0.64
      pick: 0.0
      fit: 0.28
    Nimble Ring:
      total: 0.51
      efficiency: 0.49
      win: 0.64
      pick: 0.0
      fit: 0.33
    Bragi's Harp:
      total: 0.51
      efficiency: 0.44
      win: 0.64
      pick: 0.0
      fit: 0.47
    Freya's Tears:
      total: 0.58
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.28
    Gluttonous Grimoire:
      total: 0.59
      efficiency: 0.66
      win: 0.64
      pick: 0.0
      fit: 0.46
    Obsidian Shard:
      total: 0.69
      efficiency: 0.54
      win: 1.0
      pick: 0.25
      fit: 0.22
  starter: *id001
- source: suggested
  archetype: cooldown
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Freya's Tears
  - Shifter's Shield
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Soul Gem
  - Shifter's Shield
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Kinetic Cuirass — magical protection
    swap_item: Kinetic Cuirass
  - vs_tag: physical_heavy
    swap: Shield of the Phoenix — physical protection
    swap_item: Shield of the Phoenix
  - vs_tag: sustain
    swap: Stygian Anchor — anti-heal
    swap_item: Stygian Anchor
  rationale: 'Ability-uptime skew — Cooldown Rate is a rate, not a reduction (efficiency
    + fit + win/pick). Underrated for this god: Soul Gem, Gluttonous Grimoire, Shield
    of the Phoenix, Kinetic Cuirass, Amanita Charm, Oni Hunter''s Garb, Chronos''
    Pendant, Screeching Gargoyle, Chandra''s Grace, Spear of the Magus, Gladiator''s
    Shield, Spectral Armor, The Cosmic Horror, Erosion, Eye of Providence, Helm of
    Radiance, Gem of Focus, Prophetic Cloak, Draconic Scale, Totem of Death, Rod of
    Asclepius, Leviathan''s Hide, Spear of Desolation.'
  slot_scores:
    Genji's Guard:
      total: 0.61
      efficiency: 0.71
      win: 0.63
      pick: 0.19
      fit: 0.43
    Breastplate of Valor:
      total: 0.61
      efficiency: 0.66
      win: 0.67
      pick: 0.17
      fit: 0.43
    Freya's Tears:
      total: 0.62
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.56
    Shifter's Shield:
      total: 0.57
      efficiency: 0.52
      win: 0.71
      pick: 0.16
      fit: 0.39
    Obsidian Shard:
      total: 0.69
      efficiency: 0.54
      win: 1.0
      pick: 0.25
      fit: 0.23
    Soul Gem:
      total: 0.59
      efficiency: 0.59
      win: 0.64
      pick: 0.0
      fit: 0.63
  starter: *id001
- source: suggested
  archetype: model
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Kinetic Cuirass
  - Freya's Tears
  - Gluttonous Grimoire
  - Spear of the Magus
  flex_slots:
  - Freya's Tears
  - Spear of the Magus
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Amanita Charm — magical protection
    swap_item: Amanita Charm
  - vs_tag: physical_heavy
    swap: Shield of the Phoenix — physical protection
    swap_item: Shield of the Phoenix
  - vs_tag: sustain
    swap: Stygian Anchor — anti-heal
    swap_item: Stygian Anchor
  rationale: 'The model''s own answer — no meta signal (efficiency + fit + win/pick).
    Underrated for this god: Gluttonous Grimoire, Soul Gem, Kinetic Cuirass, Amanita
    Charm, Shield of the Phoenix, Spear of the Magus, Oni Hunter''s Garb, The Cosmic
    Horror, Spear of Desolation, Helm of Radiance, Erosion, Eye of Providence, Spectral
    Armor, Rod of Asclepius, Draconic Scale, Chronos'' Pendant, Jade Scepter, Leviathan''s
    Hide, Ethereal Staff, Wish-Granting Pearl, Midgardian Mail, Mantle Of Discord.'
  slot_scores:
    Genji's Guard:
      total: 0.59
      efficiency: 0.71
      win: 0.63
      pick: 0.19
      fit: 0.31
    Breastplate of Valor:
      total: 0.59
      efficiency: 0.66
      win: 0.67
      pick: 0.17
      fit: 0.31
    Kinetic Cuirass:
      total: 0.58
      efficiency: 0.59
      win: 0.64
      pick: 0.0
      fit: 0.59
    Freya's Tears:
      total: 0.61
      efficiency: 0.59
      win: 0.71
      pick: 0.22
      fit: 0.48
    Gluttonous Grimoire:
      total: 0.61
      efficiency: 0.66
      win: 0.64
      pick: 0.0
      fit: 0.62
    Spear of the Magus:
      total: 0.55
      efficiency: 0.6
      win: 0.64
      pick: 0.0
      fit: 0.37
  starter: *id001
---
