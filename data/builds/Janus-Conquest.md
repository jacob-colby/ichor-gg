---
type: smite-build
god: Janus
mode: Conquest
builds:
- source: community
  aspect: null
  aspect_pick_rate: null
  aspect_win_rate: null
  slot_order:
  - name: Spear of Desolation
    pick_rate: 0.53
    win_rate: 0.58
    alternates:
    - name: Book of Thoth
      pick_rate: 0.31
      win_rate: 0.56
    - name: Chronos' Pendant
      pick_rate: 0.06
      win_rate: 0.64
  - name: Book of Thoth
    pick_rate: 0.32
    win_rate: 0.53
    alternates:
    - name: Spear of Desolation
      pick_rate: 0.27
      win_rate: 0.55
    - name: Soul Gem
      pick_rate: 0.08
      win_rate: 0.47
  - name: Soul Gem
    pick_rate: 0.4
    win_rate: 0.55
    alternates:
    - name: Rod of Tahuti
      pick_rate: 0.1
      win_rate: 0.58
    - name: Spear of Desolation
      pick_rate: 0.08
      win_rate: 0.45
  - name: Rod of Tahuti
    pick_rate: 0.33
    win_rate: 0.51
    alternates:
    - name: Obsidian Shard
      pick_rate: 0.26
      win_rate: 0.58
    - name: Soul Gem
      pick_rate: 0.1
      win_rate: 0.68
  - name: Obsidian Shard
    pick_rate: 0.26
    win_rate: 0.74
    alternates:
    - name: Rod of Tahuti
      pick_rate: 0.2
      win_rate: 0.71
    - name: Evil Eye
      pick_rate: 0.15
      win_rate: 0.37
  - name: Evil Eye
    pick_rate: 0.13
    win_rate: 0.65
    alternates:
    - name: Rod of Tahuti
      pick_rate: 0.11
      win_rate: 0.79
    - name: Void Shard
      pick_rate: 0.09
      win_rate: 0.45
  community_starters:
  - name: Archmage's Gem
    pick_rate: 0.48
    win_rate: 0.62
  - name: Conduit Gem
    pick_rate: 0.27
    win_rate: 0.44
  - name: Pendulum of the Ages
    pick_rate: 0.14
    win_rate: 0.73
  source_url: https://smitebrain.com/gods/janus/
  last_verified: '2026-08-14'
  god_win_rate: 0.5606694560669456
  god_matches_won: 134
  god_matches_played: 239
  god_division: obsidian
  god_window_start: '2026-08-11'
  god_window_end: '2026-08-14'
  god_matches_analyzed: 3454
  starter:
    base: Conduit Gem
    upgrade: Archmage's Gem
- source: suggested
  archetype: core
  slot_order:
  - Chronos' Pendant
  - Spear of Desolation
  - The Cosmic Horror
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Chronos' Pendant
  - The Cosmic Horror
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Top weighted-score core (efficiency + fit + win/pick). Underrated for
    this god: Gluttonous Grimoire, Chronos'' Pendant, The Cosmic Horror, Spear of
    the Magus, Nimble Ring, Doom Orb, The World Stone, Genji''s Guard, Gem of Focus,
    Dreamer''s Idol, Totem of Death, Death Metal, Helm of Radiance, Blood-Bound Book,
    Rod of Asclepius, Breastplate of Valor, Bancroft''s Talon, Jade Scepter, Bracer
    of The Abyss, Ethereal Staff, Alchemist Coat, Freya''s Tears.'
  slot_scores:
    Chronos' Pendant:
      total: 0.58
      efficiency: 0.55
      win: 0.64
      pick: 0.06
      fit: 0.67
    Spear of Desolation:
      total: 0.62
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 1.0
    The Cosmic Horror:
      total: 0.57
      efficiency: 0.58
      win: 0.56
      pick: 0.0
      fit: 0.78
    Rod of Tahuti:
      total: 0.64
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.78
    Obsidian Shard:
      total: 0.66
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.78
    Soul Gem:
      total: 0.63
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 1.0
  starter: &id001
    base: Conduit Gem
    upgrade: Archmage's Gem
- source: suggested
  archetype: mana-stack
  slot_order:
  - Genji's Guard
  - Breastplate of Valor
  - Spear of Desolation
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Genji's Guard
  - Breastplate of Valor
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Oni Hunter's Garb — magical protection
    swap_item: Oni Hunter's Garb
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'mana-stack (efficiency + fit + win/pick). Underrated for this god: Gluttonous
    Grimoire, Chronos'' Pendant, Nimble Ring, Death Metal, Genji''s Guard, The Cosmic
    Horror, Spear of the Magus, Breastplate of Valor, Doom Orb, Bragi''s Harp, The
    World Stone, Bancroft''s Talon, Triton''s Conch, Helm of Radiance, Gem of Focus,
    Blood-Bound Book, Rod of Asclepius, Totem of Death, Oni Hunter''s Garb, Polynomicon,
    Jade Scepter, Freya''s Tears.'
  slot_scores:
    Genji's Guard:
      total: 0.53
      efficiency: 0.69
      win: 0.56
      pick: 0.0
      fit: 0.24
    Breastplate of Valor:
      total: 0.51
      efficiency: 0.64
      win: 0.56
      pick: 0.0
      fit: 0.24
    Spear of Desolation:
      total: 0.56
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 0.61
    Rod of Tahuti:
      total: 0.61
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.57
    Obsidian Shard:
      total: 0.61
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.47
    Soul Gem:
      total: 0.58
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 0.71
  starter: *id001
- source: suggested
  archetype: burst
  slot_order:
  - Chronos' Pendant
  - Spear of Desolation
  - The Cosmic Horror
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Chronos' Pendant
  - The Cosmic Horror
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Ability / burst skew (efficiency + fit + win/pick). Underrated for this
    god: Gluttonous Grimoire, Chronos'' Pendant, The Cosmic Horror, Spear of the Magus,
    Genji''s Guard, Nimble Ring, Death Metal, Breastplate of Valor, Doom Orb, The
    World Stone, Freya''s Tears, Gem of Focus, Dreamer''s Idol, Totem of Death, Shield
    of the Phoenix, Bragi''s Harp, Helm of Radiance, Blood-Bound Book, Rod of Asclepius,
    Kinetic Cuirass, Amanita Charm, Screeching Gargoyle, Triton''s Conch.'
  slot_scores:
    Chronos' Pendant:
      total: 0.55
      efficiency: 0.55
      win: 0.64
      pick: 0.06
      fit: 0.44
    Spear of Desolation:
      total: 0.59
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 0.78
    The Cosmic Horror:
      total: 0.54
      efficiency: 0.58
      win: 0.56
      pick: 0.0
      fit: 0.56
    Rod of Tahuti:
      total: 0.61
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.56
    Obsidian Shard:
      total: 0.63
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.56
    Soul Gem:
      total: 0.61
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 0.88
  starter: *id001
- source: suggested
  archetype: bruiser
  slot_order:
  - Kinetic Cuirass
  - Spear of Desolation
  - Rod of Tahuti
  - Obsidian Shard
  - Amanita Charm
  - Soul Gem
  flex_slots:
  - Kinetic Cuirass
  - Spear of Desolation
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Mantle Of Discord — CC-immunity / cleanse
    swap_item: Mantle Of Discord
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Shield of the Phoenix — physical protection
    swap_item: Shield of the Phoenix
  - vs_tag: sustain
    swap: Stygian Anchor — anti-heal
    swap_item: Stygian Anchor
  rationale: 'Lifesteal bruiser skew (efficiency + fit + win/pick). Underrated for
    this god: Amanita Charm, Gluttonous Grimoire, Shield of the Phoenix, Kinetic Cuirass,
    Rod of Asclepius, Genji''s Guard, Chandra''s Grace, Nimble Ring, Freya''s Tears,
    Oni Hunter''s Garb, Blood-Bound Book, Lifebinder, Breastplate of Valor, Bancroft''s
    Talon, Phoenix Feather, Erosion, The Cosmic Horror, Shifter''s Shield, Spectral
    Armor, Eye of Providence, Spear of the Magus, Draconic Scale, Helm of Radiance,
    Jade Scepter.'
  slot_scores:
    Kinetic Cuirass:
      total: 0.55
      efficiency: 0.59
      win: 0.56
      pick: 0.0
      fit: 0.62
    Spear of Desolation:
      total: 0.54
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 0.48
    Rod of Tahuti:
      total: 0.58
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.37
    Obsidian Shard:
      total: 0.6
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.37
    Amanita Charm:
      total: 0.6
      efficiency: 0.63
      win: 0.56
      pick: 0.0
      fit: 0.82
    Soul Gem:
      total: 0.61
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 0.88
  starter: *id001
- source: suggested
  archetype: anti-tank
  slot_order:
  - Spear of Desolation
  - The Cosmic Horror
  - Spear of the Magus
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - The Cosmic Horror
  - Spear of the Magus
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Full-penetration anti-tank skew (efficiency + fit + win/pick). Underrated
    for this god: Gluttonous Grimoire, The Cosmic Horror, Spear of the Magus, Chronos''
    Pendant, Doom Orb, The World Stone, Nimble Ring, Dreamer''s Idol, Genji''s Guard,
    Screeching Gargoyle, Breastplate of Valor, Death Metal, Helm of Radiance, Gem
    of Focus, Blood-Bound Book, Rod of Asclepius, Totem of Death, Bancroft''s Talon,
    Jade Scepter, Freya''s Tears, Stone of Binding, Bracer of The Abyss, Ethereal
    Staff.'
  slot_scores:
    Spear of Desolation:
      total: 0.62
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 1.0
    The Cosmic Horror:
      total: 0.59
      efficiency: 0.58
      win: 0.56
      pick: 0.0
      fit: 0.85
    Spear of the Magus:
      total: 0.58
      efficiency: 0.57
      win: 0.56
      pick: 0.0
      fit: 0.85
    Rod of Tahuti:
      total: 0.65
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.85
    Obsidian Shard:
      total: 0.67
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.85
    Soul Gem:
      total: 0.63
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 1.0
  starter: *id001
- source: suggested
  archetype: attack-speed
  slot_order:
  - Bracer of The Abyss
  - Nimble Ring
  - Bragi's Harp
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Bragi's Harp
  - Bracer of The Abyss
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Basic-attack DPS skew (efficiency + fit + win/pick). Underrated for
    this god: Nimble Ring, Gluttonous Grimoire, Chronos'' Pendant, Genji''s Guard,
    The Cosmic Horror, Spear of the Magus, Bragi''s Harp, Death Metal, Bracer of The
    Abyss, Breastplate of Valor, Blood-Bound Book, Bancroft''s Talon, Kinetic Cuirass,
    Amanita Charm, Helm of Radiance, Freya''s Tears, Rod of Asclepius, Eye of Providence,
    Oni Hunter''s Garb, Shield of the Phoenix, Jade Scepter, Gem of Focus, Doom Orb.'
  slot_scores:
    Bracer of The Abyss:
      total: 0.49
      efficiency: 0.51
      win: 0.56
      pick: 0.0
      fit: 0.41
    Nimble Ring:
      total: 0.57
      efficiency: 0.68
      win: 0.56
      pick: 0.0
      fit: 0.49
    Bragi's Harp:
      total: 0.5
      efficiency: 0.42
      win: 0.56
      pick: 0.0
      fit: 0.64
    Rod of Tahuti:
      total: 0.57
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.3
    Obsidian Shard:
      total: 0.59
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.3
    Soul Gem:
      total: 0.56
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 0.57
  starter: *id001
- source: suggested
  archetype: cooldown
  slot_order:
  - Genji's Guard
  - Chronos' Pendant
  - Spear of Desolation
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Rod of Tahuti
  - Genji's Guard
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Freya's Tears — magical protection
    swap_item: Freya's Tears
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'Ability-uptime skew — Cooldown Rate is a rate, not a reduction (efficiency
    + fit + win/pick). Underrated for this god: Chronos'' Pendant, Genji''s Guard,
    Breastplate of Valor, Gluttonous Grimoire, Freya''s Tears, Shield of the Phoenix,
    Gem of Focus, Totem of Death, Nimble Ring, Screeching Gargoyle, Chandra''s Grace,
    The Cosmic Horror, Spear of the Magus, Death Metal, Gladiator''s Shield, Prophetic
    Cloak, Kinetic Cuirass, Amanita Charm, Helm of Radiance, Eye of Providence, Doom
    Orb, Oni Hunter''s Garb, Blood-Bound Book.'
  slot_scores:
    Genji's Guard:
      total: 0.57
      efficiency: 0.69
      win: 0.56
      pick: 0.0
      fit: 0.5
    Chronos' Pendant:
      total: 0.58
      efficiency: 0.55
      win: 0.64
      pick: 0.06
      fit: 0.65
    Spear of Desolation:
      total: 0.6
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 0.85
    Rod of Tahuti:
      total: 0.58
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.35
    Obsidian Shard:
      total: 0.6
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.35
    Soul Gem:
      total: 0.62
      efficiency: 0.57
      win: 0.55
      pick: 0.62
      fit: 0.95
  starter: *id001
- source: suggested
  archetype: model
  slot_order:
  - Gluttonous Grimoire
  - Spear of Desolation
  - The Cosmic Horror
  - Spear of the Magus
  - Rod of Tahuti
  - Obsidian Shard
  flex_slots:
  - Spear of the Magus
  - Obsidian Shard
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'The model''s own answer — no meta signal (efficiency + fit + win/pick).
    Underrated for this god: Gluttonous Grimoire, The Cosmic Horror, Spear of the
    Magus, Nimble Ring, Chronos'' Pendant, Doom Orb, The World Stone, Genji''s Guard,
    Gem of Focus, Dreamer''s Idol, Totem of Death, Death Metal, Helm of Radiance,
    Blood-Bound Book, Rod of Asclepius, Breastplate of Valor, Bancroft''s Talon, Jade
    Scepter, Bracer of The Abyss, Ethereal Staff, Alchemist Coat, Freya''s Tears.'
  slot_scores:
    Gluttonous Grimoire:
      total: 0.61
      efficiency: 0.63
      win: 0.56
      pick: 0.0
      fit: 0.88
    Spear of Desolation:
      total: 0.62
      efficiency: 0.52
      win: 0.58
      pick: 0.53
      fit: 1.0
    The Cosmic Horror:
      total: 0.57
      efficiency: 0.58
      win: 0.56
      pick: 0.0
      fit: 0.78
    Spear of the Magus:
      total: 0.57
      efficiency: 0.57
      win: 0.56
      pick: 0.0
      fit: 0.78
    Rod of Tahuti:
      total: 0.64
      efficiency: 0.77
      win: 0.51
      pick: 0.55
      fit: 0.78
    Obsidian Shard:
      total: 0.66
      efficiency: 0.52
      win: 0.74
      pick: 0.56
      fit: 0.78
  starter: *id001
---
