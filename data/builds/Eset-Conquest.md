---
type: smite-build
god: Eset
mode: Conquest
builds:
- source: community
  aspect: Aspect of the Lifewarden
  aspect_pick_rate: 0.55
  aspect_win_rate: 0.57
  slot_order:
  - name: Lifebinder
    pick_rate: 0.17
    win_rate: 0.71
    alternates:
    - name: Spear of Desolation
      pick_rate: 0.14
      win_rate: 0.67
    - name: Book of Thoth
      pick_rate: 0.14
      win_rate: 0.33
  - name: Spear of Desolation
    pick_rate: 0.14
    win_rate: 0.33
    alternates:
    - name: Genji's Guard
      pick_rate: 0.1
      win_rate: 1.0
    - name: Soul Gem
      pick_rate: 0.07
      win_rate: 0.67
  - name: Freya's Tears
    pick_rate: 0.15
    win_rate: 0.5
    alternates:
    - name: Genji's Guard
      pick_rate: 0.13
      win_rate: 0.4
    - name: Rod of Tahuti
      pick_rate: 0.13
      win_rate: 0.4
  - name: Obsidian Shard
    pick_rate: 0.13
    win_rate: 0.6
    alternates:
    - name: Rod of Tahuti
      pick_rate: 0.13
      win_rate: 0.2
    - name: Chandra's Grace
      pick_rate: 0.05
      win_rate: 0.5
  - name: The World Stone
    pick_rate: 0.11
    win_rate: 0.67
    alternates:
    - name: Void Shard
      pick_rate: 0.07
      win_rate: 0.5
    - name: Divine Ruin
      pick_rate: 0.07
      win_rate: 1.0
  - name: Evil Eye
    pick_rate: 0.12
    win_rate: 1.0
    alternates:
    - name: Divine Ruin
      pick_rate: 0.12
      win_rate: 0.0
    - name: Captain's Ring
      pick_rate: 0.06
      win_rate: 1.0
  community_starters:
  - name: Archmage's Gem
    pick_rate: 0.24
    win_rate: 0.6
  - name: Bumba's Hammer
    pick_rate: 0.19
    win_rate: 0.88
  - name: Sands Of Time
    pick_rate: 0.12
    win_rate: 0.2
  source_url: https://smitebrain.com/gods/eset/
  last_verified: '2026-09-09'
  god_win_rate: 0.5714285714285714
  god_matches_won: 24
  god_matches_played: 42
  god_division: obsidian
  god_window_start: '2026-09-08'
  god_window_end: '2026-09-09'
  god_matches_analyzed: 921
  starter:
    base: Conduit Gem
    upgrade: Archmage's Gem
- source: suggested
  archetype: core
  slot_order:
  - Doom Orb
  - Spear of the Magus
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Doom Orb
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Freya's Tears — magical protection
    swap_item: Freya's Tears
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Contagion — anti-heal
    swap_item: Contagion
  rationale: 'Top weighted-score core (efficiency + fit + win/pick). Underrated for
    this god: Soul Gem, Spear of the Magus, Gluttonous Grimoire, Doom Orb, Dreamer''s
    Idol, Chronos'' Pendant, The Cosmic Horror, Gem of Focus, Ancient Signet, Totem
    of Death, Rod of Asclepius, Polynomicon, Blood-Bound Book, Soul Reaver, Jade Scepter,
    Bancroft''s Talon, Helm of Radiance, Ethereal Staff, Staff of Myrddin, Wish-Granting
    Pearl, Typhon’s Heart, Bracer of The Abyss, Nimble Ring, Book of Thoth.'
  slot_scores:
    Doom Orb:
      total: 0.53
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.77
    Spear of the Magus:
      total: 0.55
      efficiency: 0.6
      win: 0.5
      pick: 0.0
      fit: 0.77
    The World Stone:
      total: 0.61
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.77
    Rod of Tahuti:
      total: 0.52
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.77
    Obsidian Shard:
      total: 0.6
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.87
    Soul Gem:
      total: 0.64
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 1.0
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: &id001
    base: Conduit Gem
    upgrade: Archmage's Gem
- source: suggested
  archetype: mana-stack
  slot_order:
  - Doom Orb
  - Spear of the Magus
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Doom Orb
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Contagion — anti-heal
    swap_item: Contagion
  rationale: 'mana-stack (efficiency + fit + win/pick). Underrated for this god: Soul
    Gem, Spear of the Magus, Doom Orb, Death Metal, Gluttonous Grimoire, Ancient Signet,
    Chronos'' Pendant, Dreamer''s Idol, Bragi''s Harp, Gem of Focus, Polynomicon,
    Soul Reaver, The Cosmic Horror, Rod of Asclepius, Bancroft''s Talon, Totem of
    Death, Triton''s Conch, Blood-Bound Book, Jade Scepter, Wish-Granting Pearl, Helm
    of Radiance, Breastplate of Valor, Ethereal Staff, Book of Thoth.'
  slot_scores:
    Doom Orb:
      total: 0.5
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.57
    Spear of the Magus:
      total: 0.51
      efficiency: 0.6
      win: 0.5
      pick: 0.0
      fit: 0.47
    The World Stone:
      total: 0.58
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.57
    Rod of Tahuti:
      total: 0.49
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.57
    Obsidian Shard:
      total: 0.56
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.57
    Soul Gem:
      total: 0.59
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 0.71
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: burst
  slot_order:
  - Doom Orb
  - Spear of the Magus
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Doom Orb
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Freya's Tears — magical protection
    swap_item: Freya's Tears
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Contagion — anti-heal
    swap_item: Contagion
  rationale: 'Ability / burst skew (efficiency + fit + win/pick). Underrated for this
    god: Soul Gem, Spear of the Magus, Gluttonous Grimoire, Doom Orb, Dreamer''s Idol,
    Chronos'' Pendant, Death Metal, Gem of Focus, The Cosmic Horror, Ancient Signet,
    Bragi''s Harp, Totem of Death, Rod of Asclepius, Polynomicon, Blood-Bound Book,
    Soul Reaver, Jade Scepter, Triton''s Conch, Breastplate of Valor, Bancroft''s
    Talon, Helm of Radiance, Ethereal Staff, Book of Thoth.'
  slot_scores:
    Doom Orb:
      total: 0.49
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.56
    Spear of the Magus:
      total: 0.52
      efficiency: 0.6
      win: 0.5
      pick: 0.0
      fit: 0.56
    The World Stone:
      total: 0.58
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.56
    Rod of Tahuti:
      total: 0.48
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.56
    Obsidian Shard:
      total: 0.57
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.66
    Soul Gem:
      total: 0.62
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 0.88
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: bruiser
  slot_order:
  - Freya's Tears
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Lifebinder
  - Soul Gem
  flex_slots:
  - Freya's Tears
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Amanita Charm — magical protection
    swap_item: Amanita Charm
  - vs_tag: physical_heavy
    swap: Shield of the Phoenix — physical protection
    swap_item: Shield of the Phoenix
  - vs_tag: sustain
    swap: Stygian Anchor — anti-heal
    swap_item: Stygian Anchor
  rationale: 'Lifesteal bruiser skew (efficiency + fit + win/pick). Underrated for
    this god: Soul Gem, Amanita Charm, Rod of Asclepius, Shield of the Phoenix, Gluttonous
    Grimoire, Kinetic Cuirass, Ethereal Staff, Spear of the Magus, Breastplate of
    Valor, Shifter''s Shield, Helm of Radiance, Sphere of Negation, Yogi''s Necklace,
    Erosion, Chandra''s Grace, Eye of Providence, Phoenix Feather, Draconic Scale,
    Jade Scepter, Wish-Granting Pearl, Blood-Bound Book, Doom Orb, Glorious Pridwen.'
  slot_scores:
    Freya's Tears:
      total: 0.51
      efficiency: 0.61
      win: 0.5
      pick: 0.23
      fit: 0.42
    The World Stone:
      total: 0.55
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.38
    Rod of Tahuti:
      total: 0.46
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.38
    Obsidian Shard:
      total: 0.54
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.48
    Lifebinder:
      total: 0.59
      efficiency: 0.46
      win: 0.71
      pick: 0.17
      fit: 0.7
    Soul Gem:
      total: 0.62
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 0.9
  community_ordered:
  - Freya's Tears
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Lifebinder
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: anti-tank
  slot_order:
  - Doom Orb
  - Spear of the Magus
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Doom Orb
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Screeching Gargoyle — magical protection
    swap_item: Screeching Gargoyle
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Contagion — anti-heal
    swap_item: Contagion
  rationale: 'Full-penetration anti-tank skew (efficiency + fit + win/pick). Underrated
    for this god: Soul Gem, Spear of the Magus, Gluttonous Grimoire, Doom Orb, Dreamer''s
    Idol, The Cosmic Horror, Chronos'' Pendant, Ancient Signet, Gem of Focus, Totem
    of Death, Rod of Asclepius, Polynomicon, Blood-Bound Book, Soul Reaver, Jade Scepter,
    Bancroft''s Talon, Helm of Radiance, Screeching Gargoyle, Ethereal Staff, Wish-Granting
    Pearl, Typhon’s Heart, Breastplate of Valor, Staff of Myrddin, Book of Thoth.'
  slot_scores:
    Doom Orb:
      total: 0.54
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.84
    Spear of the Magus:
      total: 0.56
      efficiency: 0.6
      win: 0.5
      pick: 0.0
      fit: 0.84
    The World Stone:
      total: 0.62
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.84
    Rod of Tahuti:
      total: 0.53
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.84
    Obsidian Shard:
      total: 0.61
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.94
    Soul Gem:
      total: 0.64
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 1.0
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: attack-speed
  slot_order:
  - Bracer of The Abyss
  - Nimble Ring
  - Bragi's Harp
  - The World Stone
  - Rod of Tahuti
  - Soul Gem
  flex_slots:
  - Bracer of The Abyss
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Freya's Tears — magical protection
    swap_item: Freya's Tears
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Contagion — anti-heal
    swap_item: Contagion
  rationale: 'Basic-attack DPS skew (efficiency + fit + win/pick). Underrated for
    this god: Soul Gem, Nimble Ring, Gluttonous Grimoire, Spear of the Magus, Bragi''s
    Harp, Bracer of The Abyss, Doom Orb, Chronos'' Pendant, Ancient Signet, Blood-Bound
    Book, Dreamer''s Idol, Death Metal, Bancroft''s Talon, Gem of Focus, Rod of Asclepius,
    The Cosmic Horror, Polynomicon, Typhon’s Heart, Soul Reaver, Totem of Death, Jade
    Scepter, Helm of Radiance, Daybreak Gavel, Book of Thoth.'
  slot_scores:
    Bracer of The Abyss:
      total: 0.47
      efficiency: 0.52
      win: 0.5
      pick: 0.0
      fit: 0.4
    Nimble Ring:
      total: 0.52
      efficiency: 0.65
      win: 0.5
      pick: 0.0
      fit: 0.48
    Bragi's Harp:
      total: 0.47
      efficiency: 0.44
      win: 0.5
      pick: 0.0
      fit: 0.62
    The World Stone:
      total: 0.54
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.31
    Rod of Tahuti:
      total: 0.45
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.31
    Soul Gem:
      total: 0.59
      efficiency: 0.57
      win: 0.67
      pick: 0.1
      fit: 0.58
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: cooldown
  slot_order:
  - Chronos' Pendant
  - Gem of Focus
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Gem of Focus
  - Rod of Tahuti
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Freya's Tears — magical protection
    swap_item: Freya's Tears
  - vs_tag: physical_heavy
    swap: Breastplate of Valor — physical protection
    swap_item: Breastplate of Valor
  - vs_tag: sustain
    swap: Contagion — anti-heal
    swap_item: Contagion
  rationale: 'Ability-uptime skew — Cooldown Rate is a rate, not a reduction (efficiency
    + fit + win/pick). Underrated for this god: Soul Gem, Chronos'' Pendant, Gem of
    Focus, Spear of the Magus, Gluttonous Grimoire, Totem of Death, Doom Orb, Breastplate
    of Valor, Dreamer''s Idol, Ancient Signet, Death Metal, Staff of Myrddin, The
    Cosmic Horror, Eye of Erebus, Chandra''s Grace, Screeching Gargoyle, Bragi''s
    Harp, Rod of Asclepius, Polynomicon, Blood-Bound Book, Soul Reaver, Jade Scepter,
    Book of Thoth.'
  slot_scores:
    Chronos' Pendant:
      total: 0.51
      efficiency: 0.55
      win: 0.5
      pick: 0.0
      fit: 0.63
    Gem of Focus:
      total: 0.49
      efficiency: 0.5
      win: 0.5
      pick: 0.0
      fit: 0.63
    The World Stone:
      total: 0.55
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.37
    Rod of Tahuti:
      total: 0.46
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.37
    Obsidian Shard:
      total: 0.54
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.47
    Soul Gem:
      total: 0.63
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 0.96
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: strength
  slot_order:
  - Jotunn's Revenge
  - Transcendence
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Rod of Tahuti
  - Transcendence
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Shield Splitter — magical protection
    swap_item: Shield Splitter
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Off-type Strength build — this kit scales on it (efficiency + fit +
    win/pick). Underrated for this god: Soul Gem, Jotunn''s Revenge, Spear of the
    Magus, Heartseeker, Hydra''s Lament, Tyrfing, Bragi''s Harp, Tekko-Kagi, Lernaean
    Bow, Nimble Ring, Death Metal, Titan''s Bane, Gluttonous Grimoire, The Crusher,
    Silverbranch Bow, The Reaper, Golden Blade, Doom Orb, Pendulum Blade, Chronos''
    Pendant, Dreamer''s Idol, Arondight, Dominance, Avatar''s Parashu, Bracer of The
    Abyss, Ancient Signet, Transcendence, Riptalon, Runeforged Hammer, Gem of Focus,
    Toxic Blade, The Cosmic Horror, Avenging Blade, Triton''s Conch, Rod of Asclepius,
    Polynomicon, Totem of Death, Book of Thoth.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.57
      efficiency: 0.72
      win: 0.5
      pick: 0.0
      fit: 0.65
    Transcendence:
      total: 0.46
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.32
    The World Stone:
      total: 0.56
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.43
    Rod of Tahuti:
      total: 0.47
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.43
    Obsidian Shard:
      total: 0.55
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.53
    Soul Gem:
      total: 0.59
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 0.66
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: str-int
  slot_order:
  - Jotunn's Revenge
  - Transcendence
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Rod of Tahuti
  - Transcendence
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Shield Splitter — magical protection
    swap_item: Shield Splitter
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Toxic Blade — anti-heal
    swap_item: Toxic Blade
  rationale: 'Hybrid Strength + Intelligence — this kit scales on both (efficiency
    + fit + win/pick). Underrated for this god: Soul Gem, Jotunn''s Revenge, Spear
    of the Magus, Heartseeker, Gluttonous Grimoire, Hydra''s Lament, Death Metal,
    Doom Orb, Titan''s Bane, The Crusher, Dreamer''s Idol, Chronos'' Pendant, The
    Reaper, Pendulum Blade, Bragi''s Harp, Ancient Signet, Arondight, Gem of Focus,
    Avatar''s Parashu, Tyrfing, The Cosmic Horror, Transcendence, Runeforged Hammer,
    Tekko-Kagi, Rod of Asclepius, Totem of Death, Triton''s Conch, Polynomicon, Avenging
    Blade, Blood-Bound Book, Soul Reaver, Jade Scepter, Bancroft''s Talon, Golden
    Blade, Silverbranch Bow, Helm of Radiance, Book of Thoth.'
  slot_scores:
    Jotunn's Revenge:
      total: 0.58
      efficiency: 0.72
      win: 0.5
      pick: 0.0
      fit: 0.7
    Transcendence:
      total: 0.45
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.3
    The World Stone:
      total: 0.58
      efficiency: 0.52
      win: 0.67
      pick: 0.24
      fit: 0.55
    Rod of Tahuti:
      total: 0.48
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.55
    Obsidian Shard:
      total: 0.57
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.65
    Soul Gem:
      total: 0.61
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 0.8
  community_ordered:
  - The World Stone
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
- source: suggested
  archetype: model
  slot_order:
  - Spear of Desolation
  - Doom Orb
  - Spear of the Magus
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  flex_slots:
  - Obsidian Shard
  - Doom Orb
  situational_swaps:
  - vs_tag: heavy_cc
    swap: Dreamer's Idol — CC-immunity / cleanse
    swap_item: Dreamer's Idol
  - vs_tag: magic_heavy
    swap: Genji's Guard — magical protection
    swap_item: Genji's Guard
  - vs_tag: physical_heavy
    swap: Helm of Radiance — physical protection
    swap_item: Helm of Radiance
  - vs_tag: sustain
    swap: Divine Ruin — anti-heal
    swap_item: Divine Ruin
  rationale: 'The model''s own answer — no meta signal (efficiency + fit + win/pick).
    Underrated for this god: Soul Gem, Spear of the Magus, Gluttonous Grimoire, Doom
    Orb, Dreamer''s Idol, Chronos'' Pendant, The Cosmic Horror, Gem of Focus, Ancient
    Signet, Totem of Death, Rod of Asclepius, Book of Thoth, Polynomicon, Blood-Bound
    Book, Soul Reaver, Jade Scepter, Bancroft''s Talon, Helm of Radiance, Ethereal
    Staff, Staff of Myrddin, Wish-Granting Pearl, Typhon’s Heart, Bracer of The Abyss,
    Nimble Ring.'
  slot_scores:
    Spear of Desolation:
      total: 0.51
      efficiency: 0.57
      win: 0.33
      pick: 0.19
      fit: 1.0
    Doom Orb:
      total: 0.53
      efficiency: 0.53
      win: 0.5
      pick: 0.0
      fit: 0.77
    Spear of the Magus:
      total: 0.55
      efficiency: 0.6
      win: 0.5
      pick: 0.0
      fit: 0.77
    Rod of Tahuti:
      total: 0.52
      efficiency: 0.86
      win: 0.2
      pick: 0.22
      fit: 0.77
    Obsidian Shard:
      total: 0.6
      efficiency: 0.54
      win: 0.6
      pick: 0.22
      fit: 0.87
    Soul Gem:
      total: 0.64
      efficiency: 0.52
      win: 0.67
      pick: 0.1
      fit: 1.0
  community_ordered:
  - Spear of Desolation
  - Rod of Tahuti
  - Obsidian Shard
  - Soul Gem
  starter: *id001
---
