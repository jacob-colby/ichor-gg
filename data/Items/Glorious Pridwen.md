---
type: smite-item
name: Glorious Pridwen
tier: 3
cost: 2550
stats:
  Physical Protection: '20'
  Magical Protection: '20'
  Cooldown Rate: '12'
passive: 'On Use: Explode (4.8m radius): Deal Magical Damage (40% of Protections)
  Gain a Shield (125% of Protections) for 5s. When the Shield expires or breaks: Explode
  again: Deal Magic Damage (70% of your Protections). Cooldown: 45s You can own up
  to 3 active items at once'
builds_from:
- Engraved Guard
- Adroit Ring
builds_into: []
source_url: https://wiki.smite2.com/w/Glorious_Pridwen
last_verified: '2026-07-18'
---
<!-- WIKI:START -->
On Use: Explode (4.8m radius): Deal Magical Damage (40% of Protections) Gain a Shield (125% of Protections) for 5s. When the Shield expires or breaks: Explode again: Deal Magic Damage (70% of your Protections). Cooldown: 45s You can own up to 3 active items at once
<!-- WIKI:END -->
