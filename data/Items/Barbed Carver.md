---
type: smite-item
name: Barbed Carver
tier: 3
cost: 2500
stats:
  Strength: '40'
  Lifesteal: 7.5%
passive: 'Ability Hit: Target gains +1 Stack of Jagged Wounds for 5s (max 3) Triggers
  once per Ability, per target Each Stack: +8% (+1% per 10% Lifesteal) Attack and
  Ability Damage as Physical Damage.'
builds_from:
- Skeggox
- Cursed Sickle
builds_into: []
source_url: https://wiki.smite2.com/w/Barbed_Carver
last_verified: '2026-07-23'
---
<!-- WIKI:START -->
Ability Hit: Target gains +1 Stack of Jagged Wounds for 5s (max 3) Triggers once per Ability, per target Each Stack: +8% (+1% per 10% Lifesteal) Attack and Ability Damage as Physical Damage.
<!-- WIKI:END -->
