---
type: smite-item
name: Leviathan's Hide
tier: 3
cost: 2500
stats:
  Physical Protection: '45'
  Plating: '5'
  Max Health: '300'
passive: 'Damaged by God Attack: That God is -10% Strength & Attack Damage for 4s.
  Attack an Enemy God: +8 Max Health Permanently. Damaged by Enemy God: -0.5s once
  per Ability Cast or Attack. Cooldown: 10s.'
builds_from:
- Veve Charm
- Legionnaire Armor
builds_into: []
source_url: https://wiki.smite2.com/w/Leviathan's_Hide
last_verified: '2026-08-01'
---
<!-- WIKI:START -->
Damaged by God Attack: That God is -10% Strength & Attack Damage for 4s. Attack an Enemy God: +8 Max Health Permanently. Damaged by Enemy God: -0.5s once per Ability Cast or Attack. Cooldown: 10s.
<!-- WIKI:END -->
