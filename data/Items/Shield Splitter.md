---
type: smite-item
name: Shield Splitter
tier: 3
cost: 2400
stats:
  Strength: '40'
  Physical Protection: '15'
  Magical Protection: '15'
passive: 'On Use: Fire a projectile 11.2m forward. +40 (+5 per Level) True Damage
  If an enemy is Shielded: -70% Shield You gain a Shield equal to the amount of Shield
  reduced Cooldown: 15s You can own up to 3 active items at once'
builds_from:
- Skeggox
- Engraved Guard
builds_into: []
source_url: https://wiki.smite2.com/w/Shield_Splitter
last_verified: '2026-08-01'
---
<!-- WIKI:START -->
On Use: Fire a projectile 11.2m forward. +40 (+5 per Level) True Damage If an enemy is Shielded: -70% Shield You gain a Shield equal to the amount of Shield reduced Cooldown: 15s You can own up to 3 active items at once
<!-- WIKI:END -->
