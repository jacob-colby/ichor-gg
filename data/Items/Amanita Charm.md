---
type: smite-item
name: Amanita Charm
tier: 3
cost: 2350
stats:
  Physical Protection: '25'
  Magical Protection: '25'
  Max Health: '300'
  Health Regen: '4'
passive: 'On Use: Create a Healing Mushroom (6.4m radius) for 4.5s. +3% Health Heal
  every 1.5s +1 Stack of Damage Reduction per Heal 10% Attack and Ability Damage Reduction
  per Stack 5% Ultimate Damage Reduction per Stack All Stacks removed after triggering
  Cooldown: 60s You can own up to 3 active items at once'
builds_from:
- Engraved Guard
- Olmec Blue
builds_into: []
source_url: https://wiki.smite2.com/w/Amanita_Charm
last_verified: '2026-07-18'
---
<!-- WIKI:START -->
On Use: Create a Healing Mushroom (6.4m radius) for 4.5s. +3% Health Heal every 1.5s +1 Stack of Damage Reduction per Heal 10% Attack and Ability Damage Reduction per Stack 5% Ultimate Damage Reduction per Stack All Stacks removed after triggering Cooldown: 60s You can own up to 3 active items at once
<!-- WIKI:END -->
