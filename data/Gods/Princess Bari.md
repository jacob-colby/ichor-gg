---
type: smite-god
name: Princess Bari
pantheon: Korean
role: Carry Mid
specializations:
- Sharpshooter
- Mobile
- Constant Damage
damage_type: magical
release_date: March 10, 2025
base_stats:
  health:
    base: 577.0
    per_level: 94.0
  physical_prot:
    base: 18.8
    per_level: 2.76
  magical_prot:
    base: 25.6
    per_level: 1.73
  attack_speed:
    base: 1.0
    per_level: 1.4
  move_speed:
    base: 370.0
    per_level: 0.0
  attack_power:
    base: 44.0
    per_level: 2.42
  health_regen:
    base: 1.7
    per_level: 0.2
abilities:
- slot: Basic Attack
  name: Princess Bari Basic Attack
  details:
  - 'Mystic Surge: Deals 50% Damage to enemies within 2.4 meters of where the projectile
    hits'
  - Projectile stops on first target hit, and does not pass through walls
  - 'Damage Scaling: 100% Strength + 20% Intelligence + 100% Attack Damage'
  - 'Range: 8.8 meters'
  - 'Radius: 0.48 meters'
  description: Fire a projectile that deals Magical Damage to the first enemy hit
  damage_type: magical
  detail_kinds:
  - null
  - null
  - magical
  - mechanic
  - mechanic
- slot: Passive
  name: Mystic Surge
  details:
  - Gain bonus Strength, Intelligence, and Attack Speed while in your Mystic Surge
    based on the rank of Spirit's Cadence
  - Additionally, Princess Bari's abilities cost no resources to cast
  - 'Strength: 10 | 18 | 26 | 34 | 42 | 60'
  - 'Intelligence: 15 | 30 | 45 | 60 | 75 | 105'
  - 'Attack Speed: 10 | 14 | 18 | 22 | 26 | 30%'
  - 'Minion Hit: 2 Spirit'
  - 'God Hit: 5 Spirit'
  description: Gain bonus Strength , Intelligence , and Attack Speed while in your
    based on the rank of Spirit's Cadence Accumulate Spirit by dealing damage to enemies.
    Upon reaching 100 Spirit you enter for 5 seconds and can use any of your first
    3 abilities with no cooldown. Abilities used this way have a bonus effect
  detail_kinds:
  - buff
  - null
  - buff
  - buff
  - buff
  - null
  - null
- slot: 1st Ability
  name: Sacred Bell
  cooldown:
  - 13.0
  details:
  - 'Mystic Surge: +20% Lob Damage and now applies a Slow. +20% Rolling Damage and
    now applies a Stun. The Bell itself is 1.67x the size'
  - Warding Gusts will pull the Bell towards you if it is within Attack range and
    has landed before casting Warding Gusts
  - You can only have 3 Bells deployed at once
  - Pushed Bells pass through and damage all enemies, but bounce off walls
  - 'Lob Damage: 70 | 115 | 160 | 205 | 250'
  - 'Lob Damage Scaling: 75% Intelligence + 60% Strength'
  - 'Roll Damage: 45 | 75 | 105 | 135 | 165'
  - 'Roll Damage Scaling: 75% Intelligence + 60% Strength'
  - 'Slow: 20%'
  - 'Slow Duration: 1.5 seconds'
  - 'Range: 8.8 meters'
  - 'Radius: 2.4 meters'
  - 'Cooldown: 13 seconds'
  description: 'Mystic Surge: +20% Lob Damage and now applies a Slow . +20% Rolling
    Damage and now applies a Stun . The Bell itself is 1.67x the size Lob a Bell,
    dealing Magical Damage to enemies where it lands. Warding Gusts or Reverent Steps
    push the Bell, dealing Magical Damage and Slowing enemies hit'
  detail_kinds:
  - null
  - null
  - null
  - null
  - magical
  - magical
  - magical
  - magical
  - slow
  - slow
  - mechanic
  - mechanic
  - mechanic
- slot: 2nd Ability
  name: Warding Gust
  cooldown:
  - 14.0
  details:
  - -30% Damage to enemies further away
  - 'Mystic Surge: +20% Damage and stronger Knockback. Bells pushed are treated as
    Mystic Surge Bells'
  - 'Damage: 65 | 110 | 155 | 200 | 245'
  - 'Damage Scaling: 80% Intelligence + 50% Strength'
  - 'Inner Cone Range: 4.4 meters'
  - 'Outer Cone Range: 8.8 meters'
  - 'Cone Angle: 70 degrees'
  - 'Cooldown: 14 seconds'
  description: 'Mystic Surge: +20% Damage and stronger Knockback . Bells pushed are
    treated as Mystic Surge Bells Swipe your fan dealing Magical Damage to enemies
    and Knocking Back enemies who are close'
  damage_type: magical
  detail_kinds:
  - debuff
  - null
  - magical
  - magical
  - mechanic
  - mechanic
  - mechanic
  - mechanic
- slot: 3rd Ability
  name: Reverent Steps
  cooldown:
  - 17.0
  - 16.5
  - 16.0
  - 15.5
  - 15.0
  details:
  - Hitting a god grants 50 Spirit and reduces this ability's Cooldown by 25%
  - 'Mystic Surge: +20% Damage, dealt instantly in a line. Bells pushed are treated
    as Mystic Surge Bells. Gain an additional 50 Spirit and 10% Attack Speed on god
    hit.'
  - You have no strafing or backpedal penalty while firing
  - This ability passes through and damages all enemies, and passes through walls
  - 'Damage: 10 | 45 | 80 | 115 | 150'
  - 'Damage Scaling: 70% Intelligence + 45% Strength + 100% Attack Damage'
  - 'Attack Speed: 15 | 17.5 | 20 | 22.5 | 25%'
  - 'Buff Duration: 5 seconds'
  - 'Range: 8.8 meters'
  - 'Radius: 0.64 meters'
  - 'Cooldown: 17 | 16.5 | 16 | 15.5 | 15 seconds'
  description: 'Mystic Surge: +20% Damage , dealt instantly in a line. Bells pushed
    are treated as Mystic Surge Bells . Gain an additional 50 Spirit and 10% Attack
    Speed on god hit. Dance for a short burst of speed in any direction, then fire
    a projectile dealing Magical Damage when it ends. Hitting a god grants Attack
    Speed to Bari.'
  damage_type: magical
  detail_kinds:
  - null
  - null
  - null
  - null
  - magical
  - magical
  - buff
  - buff
  - mechanic
  - mechanic
  - mechanic
- slot: Ultimate
  name: Spirit's Cadence
  cooldown:
  - 110.0
  details:
  - You and allies in the area gain Guardian Spirits for 4 seconds. When taking lethal
    damage become Damage Immune, Silenced, and Disarmed for 1.5 seconds
  - Casting this ability immediately places you in Mystic Surge for 7 seconds
  - Gods can receive Guardian Spirits only once per Spirit Cadence use
  - 'Pulse Damage: 50 | 70 | 90 | 110 | 130'
  - 'Pulse Damage Scaling: 25% Intelligence + 20% Strength'
  - 'Final Pulse Damage: 100 | 140 | 180 | 220 | 260'
  - 'Final Pulse Damage Scaling: 70% Intelligence + 40% Strength'
  - 'Range: 8.8 meters'
  - 'Radius: 5.6 meters'
  - 'Cooldown: 110 seconds'
  description: You and allies in the area gain Guardian Spirits for 4 seconds. When
    taking lethal damage become Damage Immune , Silenced , and Disarmed for 1.5 seconds
    Conjure a spiritual dance that pulses 3 times. The first two pulses deal Magical
    Damage . The last pulse deals bonus Magical Damage
  detail_kinds:
  - buff
  - null
  - buff
  - magical
  - magical
  - magical
  - magical
  - mechanic
  - mechanic
  - mechanic
aspects: []
source_url: https://wiki.smite2.com/w/Princess_Bari
last_verified: '2026-08-01'
---
<!-- WIKI:START -->
- Princess Bari Basic Attack
- Mystic Surge
- Sacred Bell
- Warding Gust
- Reverent Steps
- Spirit's Cadence
<!-- WIKI:END -->
