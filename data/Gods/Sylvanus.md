---
type: smite-god
name: Sylvanus
pantheon: Roman
role: Support
specializations:
- Tank
- Healing
- Utility
- Crowd Control
damage_type: magical
release_date: November 4, 2025
base_stats:
  health:
    base: 705.27
    per_level: 101.52
  mana:
    base: 296.0
    per_level: 42.0
  physical_prot:
    base: 20.52
    per_level: 3.24
  magical_prot:
    base: 29.7
    per_level: 1.62
  attack_speed:
    base: 0.9
    per_level: 0.74
  move_speed:
    base: 370.0
    per_level: 0.0
abilities:
- slot: Basic Attack
  name: Sylvanus Basic Attack
  details:
  - Projectile can be thrown over walls
  - 'Damage Scaling: 100% Strength + 20% Intelligence + 100% Attack Damage'
  - 'Range: 6.4 meters'
  - 'Radius: 1.6 meters'
  description: Lob a projectile at a target location that explodes, dealing Magical
    Damage to all enemies
- slot: Passive
  name: Nature's Bounty
  details:
  - Reduces cooldowns by 0.5 seconds; at level 15 the reduction increases to 1 second
  - 'Mana Restore: 2.5%'
  description: When your abilities hit or are deployed, a seed pod appears. Picking
    it up restores Mana and reduces your cooldowns . Picking up 8 spawns a pod near
    Sylvanus
- slot: 1st Ability
  name: Verdant Growth
  cooldown:
  - 15.0
  - 14.0
  - 13.0
  - 12.0
  - 11.0
  cost:
  - 60.0
  - 65.0
  - 70.0
  - 75.0
  - 80.0
  details:
  - If it lands on the ground without hitting an enemy, it grows into a Flower after
    6 seconds that grants nearby allies Mana Regen
  - The pod can be destroyed by Attacks, applying Protection Reduction to the destroyer
  - Grown Flowers wilt for 6 seconds when destroyed, allowing them to still interact
    with Wisps and Nature's Grasp
  - 'Damage: 80 | 120 | 160 | 200 | 240'
  - 'Damage Scaling: 50% Intelligence'
  - 'Protections Reduced: 5 | 10 | 15 | 20 | 25 + 5%'
  - 'Debuff Duration: 5 seconds'
  - 'Root Duration: 1.5 seconds'
  - 'Mana Regen: 4'
  - 'Range: 7.2 meters'
  - 'Radius: 1.6 meters'
  - 'Cooldown: 15 | 14 | 13 | 12 | 11 seconds'
  - 'Cost: 60 | 65 | 70 | 75 | 80 mana'
  description: Throw a pod that deals Magical Damage , applies Protection Reduction
    , and Roots enemies hit
- slot: 2nd Ability
  name: Wisps
  cooldown:
  - 14.0
  cost:
  - 60.0
  - 65.0
  - 70.0
  - 75.0
  - 80.0
  details:
  - The first Wisp tick deals twice the normal amount of Healing and Damage
  - Wisps can revive Wilted Flowers or cause Living Flowers to grant their Protections
    in an aura
  - Healing and Damage are reduced by 65% when affecting Minions. Seeds are not spawned
    when hitting Minions
  - 'Heal: 8 | 10 | 12 | 14 | 16'
  - 'Heal Scaling: 3% Intelligence'
  - 'Damage: 14 | 20 | 26 | 32 | 38'
  - 'Damage Scaling: 10% Intelligence'
  - 'Protections: 10 | 15 | 20 | 25 | 30'
  - 'Protections Duration: 5 seconds'
  - 'Radius: 4.8 meters'
  - 'Cooldown: 14 seconds'
  - 'Cost: 60 | 65 | 70 | 75 | 80 mana'
  description: can revive Wilted Flowers or cause Living Flowers to grant their Protections
    in an aura Release that restores Health to allies or deals Magical Damage to enemies
    repeatedly over time. Allies gain Protections while healed
- slot: 3rd Ability
  name: Nature's Grasp
  cooldown:
  - 14.0
  - 13.0
  - 12.0
  - 11.0
  - 10.0
  cost:
  - 70.0
  - 65.0
  - 60.0
  - 55.0
  - 50.0
  details:
  - Sylvanus can pull Pods to him, granting a Mana Regen aura for 30 seconds. If Sylvanus
    drops below 40% Health, he consumes the Aura to gain Wisps
  - This ability stops on first enemy god hit, and passes through walls
  - 'Stun Duration: 1 second'
  - 'Mana Regen: 4'
  - 'Range: 9.6 meters'
  - 'Radius: 0.48 meters'
  - 'Cooldown: 14 | 13 | 12 | 11 | 10 seconds'
  - 'Cost: 70 | 65 | 60 | 55 | 50 mana'
  description: Extend a vine that Stuns the first enemy god hit and Pulls them toward
    you
- slot: Ultimate
  name: Wrath of Terra
  cooldown:
  - 90.0
  cost:
  - 100.0
  details:
  - If a Pod has been grabbed, consume it to apply Verdant Growth's Protection Reduction
  - Deals damage 6 times over 5 seconds
  - 'Damage: 60 | 75 | 90 | 105 | 120'
  - 'Damage Scaling: 25% Intelligence'
  - 'Radius: 5.6 meters'
  - 'Cooldown: 90 seconds'
  - 'Cost: 100 mana'
  description: Slam the ground, Knocking Up enemies in an area and dealing Magical
    Damage repeatedly over time
aspects:
- name: Aspect of Grover's Wrath
  kit_changes: 'Attacks: +15% damage to targets debuffed by Sylvanus. Wisp''s Heal,
    Protections, and Ultimate Knockup reduced by 40%. Nature''s Grasp tethers, dealing
    damage, Slowing, and granting Sylvanus Haste. Gain Anger (+Attack Speed) when
    Verdant Growth/Ally dies; +2 on Ultimate cast. At 4 stacks, Enrage: +Attack Speed
    and a free Wisps or Nature''s Grasp cast.'
source_url: https://wiki.smite2.com/w/Sylvanus
last_verified: '2026-07-25'
---
<!-- WIKI:START -->
- Sylvanus Basic Attack
- Nature's Bounty
- Verdant Growth
- Wisps
- Nature's Grasp
- Wrath of Terra
<!-- WIKI:END -->
