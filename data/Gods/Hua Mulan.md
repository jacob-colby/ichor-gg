---
type: smite-god
name: Hua Mulan
pantheon: Chinese
role: Solo
specializations:
- Brawler
- Tank
- Pressure
damage_type: physical
release_date: January 7, 2025
base_stats:
  health:
    base: 661.0
    per_level: 94.0
  mana:
    base: 296.0
    per_level: 42.0
  physical_prot:
    base: 20.73
    per_level: 3.45
  magical_prot:
    base: 29.58
    per_level: 1.5
  attack_speed:
    base: 0.96
    per_level: 1.4
  move_speed:
    base: 370.0
    per_level: 0.0
abilities:
- slot: Basic Attack
  name: Hua Mulan Basic Attack
  details:
  - If multiple enemies are in the area, the enemy closest to the center of the area
    will be hit
  - Has a 3 hit chain. Attacks in order of 1, 1, 1.25x damage and swing time. The
    final hit cleaves
  - 'Damage Scaling: 100% Strength + 20% Intelligence + 100% Attack Damage'
  - 'Range: 1.92 meters'
  - 'Cone Angle: 120 degrees'
  description: Deal Physical Damage to an enemy in front of you
- slot: Passive
  name: Training Arc
  details:
  - 'Sword Skill: 500, 1500, 6500 damage done'
  - 'Spear Skill: 500, 1500, 6500 damage done'
  - 'Bow Skill: 200, 500, 2000 damage done'
  - Damage to non-god targets contributes 70% less Skill progression
  - 'Cooldown Reduction: -2 seconds'
  - 'Training Grounds: Up to 20% Movement Speed'
  - 'Training Arc Cooldown: 10 seconds'
  - 'Training Grounds Cooldown: 90 seconds'
  description: 'Utilize your Sword, Spear, and Bow with your abilities to gain Skill
    towards them by damaging enemies. Your expertise in each weapon enhances their
    respective abilities. Additionally gain , reducing the cooldown of your next non-ultimate
    ability, and Training Grounds, a consumable which provides team-wide speed and
    grants Cooldown: 10 seconds'
- slot: 1st Ability
  name: Cross Strike
  cooldown:
  - 14.0
  - 13.5
  - 13.0
  - 12.5
  - 12.0
  cost:
  - 60.0
  - 65.0
  - 70.0
  - 75.0
  - 80.0
  details:
  - 'Skilled: Gain an Attack Speed increase after using this ability'
  - 'Adept: Perform a third strike in an extended cone in front of you, dealing Physical
    Damage'
  - 'Mastered: Permanently gain Attack Speed'
  - 'Damage: 85 | 105 | 125 | 145 | 165'
  - 'Damage Scaling: 30% Strength'
  - 'Adept Strike Damage: 85 | 105 | 125 | 145 | 165'
  - 'Adept Strike Damage Scaling: 30% Strength'
  - 'Attack Speed: 25%'
  - 'Buff Duration: 6 seconds'
  - 'Mastered Attack Speed: 10%'
  - 'Cone Range: 4 meters'
  - 'Extended Cone Range: 4.8 meters'
  - 'Cone Angle: 100 degrees'
  - 'Cooldown: 14 | 13.5 | 13 | 12.5 | 12 seconds'
  - 'Cost: 60 | 65 | 70 | 75 | 80 mana'
  description: Strike twice with your sword, dealing Physical Damage to all enemies
    hit Adept Strike Adept Strike
- slot: 2nd Ability
  name: Spear Thrust
  cooldown:
  - 14.0
  cost:
  - 50.0
  - 55.0
  - 60.0
  - 65.0
  - 70.0
  details:
  - 'Skilled: Heal for up to 3 enemies hit'
  - 'Adept: Strike a second time with increased range, dealing bonus Physical Damage
    if the same enemy is hit twice'
  - 'Mastered: Permanently gain Strength'
  - 'Damage: 100 | 135 | 170 | 205 | 240'
  - 'Damage Scaling: 35% Strength'
  - 'Adept Strike Damage: 20 | 30 | 40 | 50 | 60'
  - 'Adept Strike Damage Scaling: 10% Strength'
  - 'Slow: 25%'
  - 'Slow Duration: 3 seconds'
  - 'Heal: 17 | 24 | 31 | 38 | 45'
  - 'Permanent Strength: 5%'
  - 'Range: 4 meters'
  - 'Adept Range: 5.6 meters'
  - 'Radius: 0.8 meters'
  - 'Cooldown: 14 seconds'
  - 'Cost: 50 | 55 | 60 | 65 | 70 mana'
  description: Thrust your spear forward, striking enemies in front of you, Slowing
    them and dealing Physical Damage
- slot: 3rd Ability
  name: Grapple
  cooldown:
  - 16.0
  - 15.5
  - 15.0
  - 14.5
  - 14.0
  cost:
  - 60.0
  details:
  - 'Skilled: Gain a Protections buff after hitting an enemy god'
  - 'Adept: Dash can now target walls'
  - 'Mastered: Permanently gain Protections'
  - Stun is unaffected by Diminishing Returns
  - You are CC Immune and Untargetable while Dashing
  - 'Damage: 90 | 135 | 180 | 225 | 270'
  - 'Damage Scaling: 50% Strength'
  - 'Adept Slash Damage: 50 | 50 | 60 | 70 | 80'
  - 'Adept Slash Damage Scaling: 20% Strength'
  - 'Stun Duration: 1 second'
  - 'Protections: 15 | 20 | 25 | 30 | 35'
  - 'Protection Buff Duration: 6 seconds'
  - 'Permanent Protections: 5%'
  - 'Range: 9.6 meters'
  - 'Radius: 0.8 meters'
  - 'Cooldown: 16 | 15.5 | 15 | 14.5 | 14 seconds'
  - 'Cost: 60 mana'
  description: Fire an arrow with a rope attached. If it hits an enemy god, Stun them,
    deal Physical Damage , and Pull them towards you while Dashing at them and following
    up with a slash for Physical Damage
- slot: Ultimate
  name: Divine Mastery
  cooldown:
  - 90.0
  cost:
  - 60.0
  - 65.0
  - 70.0
  - 75.0
  - 80.0
  details:
  - Follow up by using divine energy to Knock Up enemies with your spear, dealing
    Physical Damage
  - Finish with a powerful projectile attack from your bow, dealing Physical Damage
  - 'Mastered Sword: Turn up to 90 degrees during the forward dash'
  - 'Mastered Spear: Knocking Up enemies adds 3 seconds to their active cooldowns'
  - 'Mastered Bow: Arrow travels 40% faster'
  - Dealing damage with any part of this ability grants Skill towards the corresponding
    weapon
  - Gain Protections for the duration of this ability
  - You can cancel this ability at any time after the initial Dash
  - 'Sword Damage: 80 | 130 | 180 | 230 | 280'
  - 'Sword Damage Scaling: 60% Strength'
  - 'Spear Damage: 100 | 140 | 180 | 220 | 260'
  - 'Spear Damage Scaling: 40% Strength'
  - 'Arrow Damage: 180 | 260 | 340 | 420 | 500'
  - 'Arrow Damage Scaling: 60% Strength'
  - 'Protections: 30%'
  - 'Root Duration: 1 second'
  - 'Dash Range: 10.4 meters'
  - 'Spear Attack Range: 10.4 meters'
  - 'Bow Shot Range: 13.6 meters'
  - 'Dash Radius: 2.4 meters'
  - 'Spear Attack Radius: 1.6 meters'
  - 'Bow Shot Radius: 0.88 meters'
  - 'Cooldown: 90 seconds'
  - 'Cost: 60 | 65 | 70 | 75 | 80 mana'
  description: Unleash your entire arsenal, Dashing forward with sword slashes that
    deal Physical Damage and Root enemies
aspects: []
source_url: https://wiki.smite2.com/w/Hua_Mulan
last_verified: '2026-07-25'
---
<!-- WIKI:START -->
- Hua Mulan Basic Attack
- Training Arc
- Cross Strike
- Spear Thrust
- Grapple
- Divine Mastery
<!-- WIKI:END -->
