---
type: smite-god
name: Gilgamesh
pantheon: Babylonian
role: Solo Jungle
specializations:
- Brawler
- Crowd Control
- Constant Damage
damage_type: physical
release_date: March 24, 2026
base_stats:
  health:
    base: 661.0
    per_level: 94.0
  mana:
    base: 296.0
    per_level: 42.0
  physical_prot:
    base: 21.64
    per_level: 3.24
  magical_prot:
    base: 29.58
    per_level: 1.5
  attack_speed:
    base: 0.98
    per_level: 1.4
  move_speed:
    base: 375.0
    per_level: 0.0
abilities:
- slot: Basic Attack
  name: Gilgamesh Basic Attack
  details:
  - If multiple enemies are in the area, the enemy closest to the center of the area
    will be hit
  - 'Damage Scaling: 100% Strength + 20% Intelligence + 100% Attack Damage'
  - 'Range: 2.56 meters'
  - 'Radius: 120 degrees'
  description: Deal Physical Damage to an enemy in front of you
- slot: Passive
  name: Epic of Gilgamesh
  details:
  - 'Sun-forged Scimitar: 5 Successful Attacks against gods while ignited 3 times'
  - 'Drop Kick: Drop Kick a target into a god 3 times'
  - 'Hero''s Advance: Kill or Assist 3 gods affected by Hero''s Advance damage bonus'
  - 'Winds of Shamash: Knockback a target off the wall 5 times'
  - 'Each Quest: 100 gold + upgrade'
  description: You wield artifacts that demand quests to unlock their true potential
- slot: 1st Ability
  name: Sun-forged Scimitar
  cooldown:
  - 12.0
  - 11.5
  - 11.0
  - 10.5
  - 10.0
  cost:
  - 50.0
  - 55.0
  - 60.0
  - 65.0
  - 70.0
  details:
  - Lane minions take increased damage
  - Your Attacks deal bonus Physical Damage based on your Max Health and extend the
    effect per hit
  - You gain Health Regen per rank
  - 'Quest Upgrade: Initial ignition extends buff on enemy hits; max duration +3 seconds'
  - Duration can be extended up to 6 seconds by attacks
  - Empowered attacks count as two hits on wards and player-made deployables
  - 'Damage: 70 | 105 | 140 | 175 | 210'
  - 'Damage Scaling: 50% Strength'
  - 'Attack Bonus Scaling: 4% Max Health'
  - 'Slow: 15 | 17.5 | 20 | 22.5 | 25%'
  - 'Slow Duration: 2 seconds'
  - 'Health Regen: 2 | 3 | 4 | 5 | 6'
  - 'Radius: 3.2 meters'
  - 'Cooldown: 12 | 11.5 | 11 | 10.5 | 10 seconds'
  - 'Cost: 50 | 55 | 60 | 65 | 70 mana'
  description: Ignite your sword for 3 seconds. Enemies around the ignition take Physical
    Damage and are Slowed
- slot: 2nd Ability
  name: Drop Kick
  cooldown:
  - 14.0
  cost:
  - 60.0
  - 65.0
  - 70.0
  - 75.0
  - 80.0
  details:
  - Enemies Knocked Forward pass through minions, but stop on walls and enemy gods
  - 'Quest Upgrade: When Stun fades, targets are Intoxicated'
  - Player-made walls also trigger the burst damage and Stun
  - 'Kick Damage: 60 | 90 | 120 | 150 | 180'
  - 'Kick Damage Scaling: 30% Strength'
  - 'Bonus Damage: 25 | 40 | 55 | 70 | 85'
  - 'Bonus Damage Scaling: 20% Strength'
  - 'Impact Damage: 50 | 95 | 140 | 185 | 230'
  - 'Impact Damage Scaling: 30% Strength'
  - 'Stun Duration: 1.2 seconds'
  - 'Intoxicate Duration: 2 seconds'
  - 'Range: 4 meters'
  - 'Knockback Distance: 9.6 meters'
  - 'Radius: 1.92 meters'
  - 'Cooldown: 14 seconds'
  - 'Cost: 60 | 65 | 70 | 75 | 80 mana'
  description: Drop kick an area in front of you, dealing Physical Damage to enemies.
    The closest enemy is Knocked Forward , taking bonus Physical Damage when hitting
    a minion or burst Physical Damage and being Stunned when hitting a god or wall.
    Launched enemies hitting Winds of Shamash are Knocked Back towards the center
    of the ring
- slot: 3rd Ability
  name: Hero's Advance
  cooldown:
  - 16.0
  cost:
  - 70.0
  details:
  - Center sweet spot applies increased Damage to enemies
  - Allies moving toward the beacon gain Movement Speed, halved once inside
  - Full Movement Speed buff range is 17.6 meters
  - 'Quest Upgrade: The beacon now travels with you'
  - 'Damage: 70 | 110 | 150 | 190 | 230'
  - 'Damage Scaling: 40% Strength'
  - 'Movement Speed: 25%'
  - 'Increased Damage Taken: 12.5%'
  - 'Debuff Duration: 5 seconds'
  - 'Range: 8.8 meters'
  - 'Radius: 4.8 meters'
  - 'Cooldown: 16 seconds'
  - 'Cost: 70 mana'
  description: Allies moving toward the beacon gain Movement Speed , halved once inside
    Leap to a target location, dealing Physical Damage on impact. Create a beacon
    for 5 seconds
- slot: Ultimate
  name: Winds of Shamash
  cooldown:
  - 90.0
  cost:
  - 80.0
  - 85.0
  - 90.0
  - 95.0
  - 100.0
  details:
  - 'Quest Upgrade: Ticks every 0.375 seconds and the outer wall Slow increases to
    70%'
  - 'Damage: 90 | 145 | 200 | 255 | 310'
  - 'Damage Scaling: 40% Strength'
  - 'Wind Damage: 16 | 20 | 24 | 28 | 32'
  - 'Wind Damage Scaling: 5% Strength'
  - 'Binding Damage: 150 | 225 | 300 | 375 | 450'
  - 'Binding Damage Scaling: 50% Strength'
  - 'Inner Slow: 20%'
  - 'Wall Slow: 40%'
  - 'Slow Duration: 2 seconds'
  - 'Range: 7.2 meters'
  - 'Radius: 5.44 meters'
  - 'Cooldown: 90 seconds'
  - 'Cost: 80 | 85 | 90 | 95 | 100 mana'
  description: Call the winds of Shamash at a target location for 6 seconds. Enemies
    inside at start take Physical Damage and are Slowed . Enemies in the ring take
    Physical Damage every 0.5 seconds. Outer wall heavily Slows ; your damage Knocks
    Back enemies towards the center. At 6 seconds, wind becomes thread, dealing bonus
    Physical Damage and Rooting enemies inside
aspects:
- name: Aspect of Shamash
  kit_changes: 'Winds of Shamash is no longer an ultimate. Place a wind wall that
    transforms once with your next ability: ignites from Sun-forged Scimitar, solidifies
    from Drop Kick, or launches allies from Hero''s Advance. It can only transform
    once, granting you Attack Speed, Tenacity, or Pathfinding until its next transform.'
source_url: https://wiki.smite2.com/w/Gilgamesh
last_verified: '2026-07-25'
---
<!-- WIKI:START -->
- Gilgamesh Basic Attack
- Epic of Gilgamesh
- Sun-forged Scimitar
- Drop Kick
- Hero's Advance
- Winds of Shamash
<!-- WIKI:END -->
